...
CATEGORIES = ("helpers", "builders", "factories", "constants", "configs", "dataclasses", "facades")


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("root", nargs="?", default=".", help="Repository root or subdirectory to scan")
    parser.add_argument("--format", choices=("text", "json"), default="text")
    parser.add_argument("--category", action="append", choices=CATEGORIES, help="Limit to one or more categories")
    parser.add_argument("--limit", type=int, default=200, help="Maximum records to print")
    parser.add_argument("--no-tests", action="store_true", help="Skip tests/spec directories")
    parser.add_argument("--grep", help="Case-insensitive substring filter over name/signature/path")
...
def print_text(items: list[Item], total_before_limit: int) -> None:
    ...
    print(f"shared utility inventory: showing {len(items)} of {total_before_limit} matches")
...
def main(argv: list[str]) -> int:
    ...
    filtered_all = filter_items(items, argparse.Namespace(**{**vars(args), "limit": 10**9}))
