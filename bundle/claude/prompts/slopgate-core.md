...
- Bundled recovery skills: `code-hygiene-refactor` for PY-CODE-012/013/017/018 and similar design smells, `hygiene-orchestrator` for broad QUALITY-LINT batches, `isx-cli` for semantic search (`sgt search`), `codegraph-cli` for indexed call-graph exploration ([CodeGraph](https://github.com/colbymchenry/codegraph); `codegraph install` + per-repo `codegraph init -i`), `rtk-cli` for compact shell output ([RTK](https://github.com/rtk-ai/rtk); `rtk init` separately), and `code-smell-utility-locator` before adding new helpers/wrappers.
