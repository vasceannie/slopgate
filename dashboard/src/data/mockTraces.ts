...
function genSessions(count: number, _startDate: Date, _endDate: Date) {
	...
	for (let i = 0; i < count; i++) {
		sessions.push(
			`sess_${i.toString(36).padStart(6, "0")}_${Math.floor(rand() * 1e6).toString(36)}`,
		);
	}
	return sessions;
}
...
export function generateMockData() {
	...
	for (const sid of sessions) {
		...
		const enforcementMode = isSlopgateRepo
			? rand() < 0.8
				? "repo_strict"
				: "repo_relaxed"
			: "outside_repo";
		...
		for (const evName of sessionEvents) {
			...
			const tool =
				evName === "SessionStart" || evName === "Stop" ? "" : pick(TOOLS);
			...
			events.push({
				...
				candidate_paths:
					evName === "SessionStart"
						? []
						: pickN(sessionPaths, 1 + Math.floor(rand() * 2)),
			...
			if (
				...
				for (let f = 0; f < findingCount; f++) {
					...
					const ruleId =
						rand() < 0.15 ? pick(DUPLICATION_RULES) : pick(ALL_RULES);
					...
					const finding: RuleFinding = {
						...
						metadata: {
							category: DUPLICATION_RULES.includes(ruleId)
								? "duplication"
								: "general",
						},
						enforcement_mode: enforcementMode,
				...
				results.push({
					...
					errors:
						rand() < 0.03
							? [`RuleEngineError: timeout evaluating ${pick(ALL_RULES)}`]
							: [],
					output:
						worstDecision === "allow"
							? null
