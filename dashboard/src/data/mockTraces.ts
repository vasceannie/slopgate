import type {
  Platform, EventName, Decision, Severity,
  HookEvent, RuleFinding, HookResult, SubprocessRun, RuntimeConfig,
} from "@/types/slopgate";
...
const EVENT_NAMES: EventName[] = ["SessionStart", "PreToolUse", "PermissionRequest", "PostToolUse", "PostToolUseFailure", "Stop"];
const DECISIONS: Decision[] = ["allow", "deny", "block", "ask", "context", "warn"];
const SEVERITIES: Severity[] = ["LOW", "MEDIUM", "HIGH", "CRITICAL"];

const TOOLS = [
  "Write", "Edit", "Read", "Bash", "Search", "Grep", "ListDir",
  "WebSearch", "TodoRead", "TodoWrite", "Glob", "Task", "MultiEdit",
];

const LANGUAGES = ["python", "typescript", "javascript", "rust", "go", "java", "ruby", "shell"];

const PATHS = [
  "src/main.py", "src/utils.ts", "lib/core.rs", "app/handler.go",
  "tests/test_main.py", "src/components/App.tsx", "scripts/deploy.sh",
  "src/models/user.py", "src/services/auth.ts", "config/settings.yaml",
];

const PYTHON_RULES = [
  "no-bare-except", "no-eval-exec", "no-hardcoded-secrets", "no-assert-in-prod",
  "no-mutable-default", "no-star-import", "no-print-statements", "no-todo-fixme",
  "no-broad-exception", "no-nested-ternary", "no-magic-numbers", "no-global-state",
  "no-circular-import", "no-unused-variable", "no-shadowed-builtin",
  "repeated-code-block", "duplicate-call-sequence", "semantic-clone",
  "no-long-function", "no-deep-nesting", "no-complex-comprehension",
  "no-unsafe-yaml", "no-pickle-loads", "no-subprocess-shell", "no-tempfile-race",
  "no-sql-injection", "no-os-system", "no-weak-crypto", "no-debug-left", "no-empty-except",
];

const REGEX_RULES = [
  "no-console-log", "no-debugger", "no-alert", "no-inline-style",
  "no-any-type", "no-ts-ignore", "no-eslint-disable", "no-fixme",
  "no-hack-comment", "no-xxx-comment", "no-password-literal", "no-api-key-literal",
  "no-localhost-url", "no-http-url", "no-hardcoded-ip", "no-empty-catch",
  "no-var-declaration", "no-document-write", "no-innerhtml", "no-onclick-handler",
  "no-jquery", "no-lodash-fp", "no-moment-js", "no-sync-xhr",
  "no-window-eval", "no-with-statement", "no-label-statement", "no-comma-operator",
  "no-void-operator", "no-bitwise-operator", "no-nested-callback",
  "no-prototype-builtin", "no-new-object", "no-new-array",
  "no-delete-operator", "no-caller-callee", "no-iterator",
  "no-restricted-globals", "no-implicit-coercion", "no-sequences",
];
...
const ASYNC_COMMANDS = [
  "ruff check --fix .", "mypy --strict .", "pytest -x --tb=short",
  "eslint --fix src/", "tsc --noEmit", "cargo clippy -- -D warnings",
  "go vet ./...", "rubocop -a", "shellcheck scripts/*.sh", "black --check .",
];

function genSessions(count: number, startDate: Date, endDate: Date) {
...
export function generateMockData() {
  ...
  for (const sid of sessions) {
    ...
    for (const evName of sessionEvents) {
      ...
      events.push({
        timestamp, platform, event_name: evName,
      ...
      if (evName === "PreToolUse" || evName === "PostToolUse" || evName === "PermissionRequest") {
        ...
        for (let f = 0; f < findingCount; f++) {
          ...
          const severity = PYTHON_RULES.includes(ruleId)
            ? pick(["MEDIUM", "HIGH", "CRITICAL"] as Severity[])
            : pick(SEVERITIES);
          const decision: Decision =
            severity === "CRITICAL" ? pick(["block", "deny"] as Decision[]) :
            severity === "HIGH" ? pick(["deny", "ask", "warn"] as Decision[]) :
            severity === "MEDIUM" ? pick(["ask", "warn", "context"] as Decision[]) :
            pick(["allow", "context", "warn"] as Decision[]);

          const finding: RuleFinding = {
            timestamp, platform, event_name: evName, session_id: sid,
            tool_name: tool, rule_id: ruleId, severity, decision,
        ...
        const worstDecision: Decision = sessionFindings.length === 0 ? "allow" :
          sessionFindings.some(f => f.decision === "block") ? "block" :
          sessionFindings.some(f => f.decision === "deny") ? "deny" :
          sessionFindings.some(f => f.decision === "ask") ? "ask" :
          sessionFindings.some(f => f.decision === "warn") ? "warn" :
          sessionFindings.some(f => f.decision === "context") ? "context" : "allow";

        const skipped = rand() < 0.05;
        results.push({
          timestamp, platform, event_name: evName, session_id: sid, tool_name: tool,
          findings: sessionFindings.map(f => ({
            rule_id: f.rule_id, severity: f.severity, decision: f.decision, message: f.message,
          })),
      ...
      if (evName === "PostToolUse" && rand() < 0.3) {
        ...
        subprocesses.push({
          ...
          event_name: "async_subprocess", session_id: sid,
          command: cmd, cwd: "/repo/" + pick(["frontend", "backend", "infra", "shared"]),
...
export const mockConfig: RuntimeConfig = {
  ...
  skip_repos: [
    "legacy-monolith", "archived-experiment",
  ],
};
