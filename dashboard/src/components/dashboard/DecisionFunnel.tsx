import type { EventName, Decision, Platform, TimeWindow } from "@/types/slopgate";
...
const PIPELINE_CELL_CLASSES: Record<Platform, readonly [string, string, string, string]> = {
  claude: ["bg-platform-claude/5 text-muted-foreground", "bg-platform-claude/20 text-platform-claude", "bg-platform-claude/40 text-foreground", "bg-platform-claude/60 text-foreground"],
  codex: ["bg-platform-codex/5 text-muted-foreground", "bg-platform-codex/20 text-platform-codex", "bg-platform-codex/40 text-foreground", "bg-platform-codex/60 text-foreground"],
  opencode: ["bg-platform-opencode/5 text-muted-foreground", "bg-platform-opencode/20 text-platform-opencode", "bg-platform-opencode/40 text-foreground", "bg-platform-opencode/60 text-foreground"],
};
...
function formatAxisLabel(time: string, window: TimeWindow): string {
  ...
  if (window === "1h") {
    ...
    return d.toLocaleDateString("en-US", { hour: "numeric", minute: "2-digit" });
  }
  if (window === "6h") {
...
function getPipelineCellClass(platform: Platform, value: number, maxValue: number): string {
  const bucket = value === 0 || maxValue === 0
    ? 0
    : Math.max(1, Math.ceil((value / maxValue) * PIPELINE_CLASS_BUCKETS));
  const bucketClass = PIPELINE_CELL_CLASSES[platform][bucket] ?? PIPELINE_CELL_CLASSES[platform][0];
  return `${PIPELINE_CELL_BASE} ${bucketClass}`;
}

function buildLanePoints(values: number[]): { max: number; latest: number; points: string } {
  ...
  const points = values.map((value, index) => {
    ...
    const y = max === 0 ? LANE_CHART_HEIGHT : LANE_CHART_HEIGHT - (value / max) * LANE_CHART_HEIGHT;
...
export function DecisionFunnel({ timeSeries, eventsByType, eventsByTypeAndPlatform, timeWindow = "7d" }: Props) {
  ...
  const axisLabels = timeSeries.length > 0 ? [
  ...
  const pipelineRows = PIPELINE_STAGES
    .map(stage => ({
      stage,
  ...
  const maxPipelineCell = Math.max(...pipelineRows.flatMap(row => row.harnesses.map(cell => cell.value)), 0);
            ...
            <div className="grid h-full grid-rows-[1fr_auto] gap-2">
              <div className="grid content-around gap-2">
                {decisionLanes.map(lane => (
                  <div key={lane.decision} className="grid grid-cols-[68px_minmax(0,1fr)_112px] items-center gap-2 text-[9px]">
                    ...
                    >
                      <line x1="0" y1={LANE_CHART_HEIGHT} x2={LANE_CHART_WIDTH} y2={LANE_CHART_HEIGHT} stroke="hsl(220, 15%, 15%)" strokeWidth="0.8" />
                      {lane.points && (
                        <polyline
                          points={lane.points}
                          fill="none"
                    ...
                    <div className="grid text-right font-mono leading-none" style={{ width: LANE_VALUE_WIDTH }}>
                      <span className="text-foreground">{formatCompactCount(lane.latest)}</span>
                      <span className="text-[8px] text-muted-foreground">max {formatCompactCount(lane.max)}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid h-full grid-rows-[auto_1fr_auto] gap-2">
              <div className="grid grid-cols-[minmax(104px,1fr)_58px_repeat(3,minmax(54px,70px))] gap-1.5 text-[8px] uppercase tracking-wide text-muted-foreground">
                ...
                {PIPELINE_PLATFORMS.map(platform => (
                  <span key={platform} className="text-center">{platform}</span>
                ))}
              </div>
              <div className="grid content-around gap-2">
                {pipelineRows.map(row => (
                  <div key={row.stage} className="grid grid-cols-[minmax(104px,1fr)_58px_repeat(3,minmax(54px,70px))] items-center gap-1.5 text-[9px]">
                    ...
                    {row.harnesses.map(cell => (
                      <div
                        ...
                        className={getPipelineCellClass(cell.platform, cell.value, maxPipelineCell)}
                        title={`${row.label} · ${cell.platform}: ${cell.value.toLocaleString()}`}
                        aria-label={`${row.label} ${cell.platform} ${cell.value.toLocaleString()}`}
                      >
