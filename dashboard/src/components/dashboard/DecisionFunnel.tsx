...
import type { Decision, EventName, Platform, TimeWindow } from "@/types/slopgate";
...
function getPipelineCellClass(platform: Platform, value: number, maxValue: number): string {
  const bucket = value === 0 || maxValue === 0 ? 0 : Math.max(1, Math.ceil((value / maxValue) * PIPELINE_CLASS_BUCKETS));
