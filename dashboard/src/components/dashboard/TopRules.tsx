			...
			? topRules
					...
					.sort(
						(a, b) =>
							(b.decisions.block || 0) +
							(b.decisions.deny || 0) -
							((a.decisions.block || 0) + (a.decisions.deny || 0)),
					)
			: view === "warning"
				? topRules
						.filter(
							(r) => (r.decisions.warn || 0) + (r.decisions.context || 0) > 0,
						)
						.sort(
							(a, b) =>
								(b.decisions.warn || 0) +
								(b.decisions.context || 0) -
								((a.decisions.warn || 0) + (a.decisions.context || 0)),
						)
				: topRules;
		...
		const outerRing: SeverityRingDatum[] = SEVERITIES.flatMap((s) => {
			...
			return sorted.map((r, _i) => ({
				...
				description: truncateDescription(
					getRuleDescription(r.rule_id) ?? r.rule_id,
				),
			}));
		});
					...
					<div className="flex min-h-0 flex-col gap-1 rounded-md border border-border bg-card/30 p-2">
							...
							{paretoData.length > 0 ? (
								<ResponsiveContainer width="100%" height="100%">
									<ComposedChart
										data={paretoData}
										layout="vertical"
										margin={{ top: 5, right: 50, bottom: 5, left: 4 }}
									>
										<XAxis
											type="number"
											domain={[0, "dataMax"]}
											...
											content={({ active, payload }) => {
												...
												return (
													<div className="bg-card border border-border rounded px-3 py-2 text-xs font-mono shadow-lg">
														...
														<div className="text-signal-allow mt-0.5">
															Cumulative: {d.cumulativePct}%
														</div>
													</div>
												);
											}}
						...
						<div className="flex items-center justify-between text-[10px] text-muted-foreground px-1">
							<span>
								...
								{filtered.length > MAX_PARETO_ITEMS &&
									` · top ${MAX_PARETO_ITEMS} shown`}
							</span>
				...
				<div className="border border-border rounded-md bg-card/30 p-3">
					...
					<div className="flex flex-wrap gap-3">
						{duplicationByRule.map((d) => (
							<div
								key={d.rule_id}
								className="flex items-center gap-2 px-3 py-2 rounded-sm bg-muted/30 border border-border"
							>
