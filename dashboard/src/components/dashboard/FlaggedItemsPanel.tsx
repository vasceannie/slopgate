import { Check, Clock, Download, Flag, Radio, Target, Trash2, X } from "lucide-react";
    ...
    <div className={cn("px-3 py-2 text-xs", flag.resolved && "opacity-50")}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 shrink-0 ml-2">
          ...
          {flag.resolved ? (
            <button type="button" onClick={onUnresolve} className="ml-1 text-muted-foreground hover:text-signal-ask" title="Reopen">
              <X className="w-3 h-3" />
            </button>
          ) : (
            <button type="button" onClick={onResolve} className="ml-1 text-muted-foreground hover:text-primary" title="Resolve">
              <Check className="w-3 h-3" />
            </button>
          )}
          <button type="button" onClick={onRemove} className="text-muted-foreground hover:text-signal-block" title="Delete">
      ...
      <div className="text-[10px] text-muted-foreground/60 mt-0.5 ml-5">{new Date(flag.createdAt).toLocaleString()}</div>
    </div>
  );
}
