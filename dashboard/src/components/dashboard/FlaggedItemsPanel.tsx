...
import type { InvestigationFlag } from "@/types/slopgate";

export function FlaggedItemsPanel() {
	const { flags, resolveFlag, unresolveFlag, removeFlag, exportFlags } =
		useFlagSystem();
		...
		return (
			<div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
				...
				<div className="text-[10px] mt-1">
					Click the flag icon on any event, rule, path, or session to flag it
				</div>
			</div>
		);
	}
		...
		<div className={cn("px-3 py-2 text-xs", flag.resolved && "opacity-50")}>
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-2 min-w-0">
					<Flag
						className="w-3 h-3 text-signal-ask shrink-0"
						fill={flag.resolved ? "none" : "currentColor"}
					/>
					<span
						className={cn("font-medium", FLAG_TARGET_LABELS[flag.target].color)}
					>
						{FLAG_TARGET_LABELS[flag.target].label}
					</span>
					<span className="text-muted-foreground">·</span>
					<span className="text-[10px] px-1 py-0.5 bg-muted rounded uppercase">
						{flag.itemType}
					</span>
				...
				<div className="flex items-center gap-1 shrink-0 ml-2">
					...
					{flag.resolved ? (
						<button
							type="button"
							onClick={onUnresolve}
							className="ml-1 text-muted-foreground hover:text-signal-ask"
							title="Reopen"
						>
							<X className="w-3 h-3" />
						</button>
			...
			{flag.notes && (
				<div className="text-[10px] text-foreground/70 mt-0.5 ml-5 italic">
					"{flag.notes}"
				</div>
			)}
