					...
					<div className="min-w-0">
						...
						<div className="text-muted-foreground text-[11px] mt-0.5 truncate">
							{isBlocked ? (
								<span>
									Blocked by rule{" "}
									<strong className="text-foreground">{cause.ruleId}</strong>:{" "}
									{cause.message}
								</span>
							) : isAdvisory ? (
								<span>
									Advisory findings triggered by{" "}
									<strong className="text-foreground">{cause.ruleId}</strong>:{" "}
									{cause.message}
								</span>
							) : (
					...
					<div>
						...
						<span
							className={cn(
								...
								PLATFORM_BADGE_STYLE[session.platform],
							)}
						>
							{session.platform}
						</span>
					...
					<div>
						...
						<span className="text-foreground font-medium">
							{session.duration > 60000
								...
								: `${(session.duration / 1000).toFixed(0)}s`}
						</span>
					</div>
				</div>
			</div>
