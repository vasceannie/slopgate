    ...
    <td className="px-2 py-2.5 text-left">
      <span
        className={cn(
          ...
          partiallyEnabled ? "bg-signal-ask" : enabled ? activeColor : "bg-muted-foreground/30",
        )}
        title={title}
      />
    </td>
...
const RuleIdentityCell = memo(function RuleIdentityCell({ ruleId, title, description, sevColor }: RuleIdentityCellProps) {
  return (
    <>
      <td className="px-3 py-2.5 font-mono text-[0.875rem] font-semibold truncate max-w-[200px]" style={{ color: sevColor }}>
        {ruleId}
      </td>
      <td className="px-3 py-2.5 max-w-xs md:max-w-md truncate">
        <div className="font-semibold text-foreground truncate">{title}</div>
        {description && <div className="text-[0.75rem] text-muted-foreground truncate mt-0.5">{description}</div>}
      </td>
    </>
  );
});
...
const RuleRow = memo(
  function RuleRow({ rule, isSelected, onSelectRule, sevColor }: RuleRowProps) {
    ...
    return (
      ...
      >
        ...
        <StatusCell
          ...
          title={rule.cliPartiallyEnabled ? "CLI partially active" : rule.cliEnabled ? "CLI active" : "CLI Off"}
        />
        <RuleIdentityCell ruleId={rule.rule_id} title={rule.title} description={rule.description} sevColor={sevColor} />
        <td className="px-3 py-2.5">
          <Badge
            variant="outline"
        ...
        <td className="px-3 py-2.5">
          <Badge
            variant="outline"
            className={cn("text-[0.75rem] px-2 py-0.5 rounded uppercase font-medium border", actionBadgeStyles[rule.action])}
          >
            {rule.action}
          </Badge>
        </td>
        <td className="px-3 py-2.5 text-right font-mono font-medium">
          {rule.fireCount > 0 ? (
            <span className="text-signal-ask font-semibold">{rule.fireCount}</span>
          ) : (
        ...
        <td className="px-3 py-2.5 text-center">
          {rule.isChanged ? (
            ...
            <span className="text-muted-foreground/20 text-[0.75rem] font-medium">—</span>
          )}
        </td>
      </tr>
    );
...
const FilterBar = memo(function FilterBar({ search, onSearchChange, activeFilter, onFilterChange, filterOptions }: FilterBarProps) {
  return (
    ...
    >
      ...
      <td colSpan={8} className="px-2 py-2 font-medium">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[0.75rem] text-muted-foreground">({rulesCount} rules)</span>
          {activeCount > 0 && (
            <Badge
              variant="outline"
...
const RuleTable = memo(function RuleTable({ grouped, expandedCategories, toggleCategory, selectedRuleId, onSelectRule }: RuleTableProps) {
  return (
...
export const RuleList = memo(function RuleList({ allRules, selectedRuleId, onSelectRule }: RuleListProps) {
  ...
  const filteredRules = useMemo(() => {
    ...
    switch (activeFilter) {
      ...
      case "disabled":
        ...
        break;
      case "partial":
        list = list.filter((r) => r.cliPartiallyEnabled || (r.hookSupported && r.cliSupported && r.hookEnabled !== r.cliEnabled));
        break;
      case "unsupported":
