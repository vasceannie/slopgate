...
export const COMMAND_ONLY_RULE_PREFIXES = [
	"GIT-",
	"REMIND-",
	"PY-SHELL-",
	"SHELL-",
...
export const CONFIG_SAFETY_RULE_PREFIXES = [
	"BASELINE-",
	"CONFIG-",
	"FE-LINTER-",
	"PY-LINTER-",
...
export function cliUnsupportedReason(
	rule_id: string,
...
export function directHookRuleCliCounterparts(
...
export function hookCounterpartsForCli(
	rule_id: string,
...
export function surfaceHookEnabled(
	config: SlopgateConfig,
	rule_id: string,
): boolean {
...
export function getCliRuleIds(
	...
	return cliCounterparts.length > 0
		? cliCounterparts
		: regexCliExecutable
			? [rule_id]
			: [];
...
export function getCliDefaultEnabled(
	rule_id: string,
	config: SlopgateConfig,
	cliRuleIds: string[],
): boolean {
...
export function getRuleChangedFields(
	rule_id: string,
	saved: SlopgateConfig,
	current: SlopgateConfig,
): string[] {
	const fields: string[] = [];
