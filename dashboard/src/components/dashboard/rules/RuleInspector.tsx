...
import type { RuleCliSurface, RuleHookSurface, RuleMetadata, RuleSurfaceAction } from "@/types/slopgate";
...
interface RuleInspectorProps {
  ...
  onSetRuleCliSurface: (id: string, cliRuleIds: string[], cli: RuleCliSurface) => void;
  ...
  return (
    <div className="space-y-1 pb-3 border-b border-border/60">
      <div className="flex items-center justify-between">
        ...
        <Badge variant="outline" className="font-mono text-[0.75rem] bg-muted/30 border-transparent text-muted-foreground">
          Source: {source}
        </Badge>
      </div>
      <h3 className="font-semibold text-foreground text-[1.125rem]">{title}</h3>
      {description && <p className="text-muted-foreground text-[0.875rem] leading-relaxed pt-1">{description}</p>}
    </div>
  );
});
...
const PlacementSection = memo(function PlacementSection({
  ...
  return (
    <div className="space-y-3">
      <h4 className="text-[0.75rem] text-muted-foreground uppercase tracking-wider font-semibold">Surface Placement</h4>
      <div className="grid grid-cols-1 gap-2.5">
        <div className="flex items-center justify-between p-2.5 rounded bg-muted/20 border border-border/40">
          <div className="flex flex-col">
            ...
            <span className="text-[0.75rem] text-muted-foreground">Triggers during live agent tool invocations</span>
          </div>
          <div>
            {hookSupported ? (
              <Switch aria-label={`${ruleId} hook enablement`} checked={hookEnabled} onCheckedChange={handleHookChange} />
            ) : (
        ...
        <div className="flex items-center justify-between p-2.5 rounded bg-muted/20 border border-border/40">
          <div className="flex flex-col">
            ...
            <span className="text-[0.75rem] text-muted-foreground">Runs inside batch lint and pre-commit scans</span>
          </div>
          <div>
            {cliSupported ? (
              <Switch aria-label={`${ruleId} CLI enablement`} checked={cliEnabled} onCheckedChange={handleCliChange} />
            ) : (
              <Badge variant="secondary" className="text-[0.75rem] px-2 py-0.5 rounded font-normal border-transparent bg-muted/40">
                {cliUnsupportedReason}
              </Badge>
...
const HookParamsSection = memo(function HookParamsSection({
  ...
  return (
    <div className="space-y-3 pt-2">
      <h4 className="text-[0.75rem] text-muted-foreground uppercase tracking-wider font-semibold">Hook Parameters</h4>
      ...
      <div className="space-y-1.5 pt-1">
        <span className="text-[0.75rem] text-muted-foreground font-medium block">Event Subscription Triggers</span>
...
const PathExclusionsSection = memo(function PathExclusionsSection({
  ...
  return (
    <div className="space-y-3 pt-2">
      <h4 className="text-[0.75rem] text-muted-foreground uppercase tracking-wider font-semibold">Rule Path Exclusions</h4>
      {source === "regex" ? (
  ...
  return (
    <>
        ...
        <div className="space-y-1.5 pt-2 border-t border-border/50 text-[0.75rem] text-muted-foreground">
          {cliCounterparts.length > 0 && (
            <div className="font-mono">
              <Badge
                variant="outline"
                className="px-1.5 py-0.5 rounded border border-border bg-muted/30 text-[0.75rem] font-normal text-muted-foreground"
              >
      ...
      <Card className="p-3 bg-card border border-border rounded-md space-y-1 shadow-none">
        ...
        <p className="text-[0.75rem] text-muted-foreground leading-relaxed">{effectiveBehaviorSummary}</p>
      </Card>
    </>
  );
});
...
function useRuleInspectorState(
  ...
  const removeExclusion = useCallback(
    (glob: string) => {
      ...
      onExclusionsChange(
        rule.rule_id,
        rule.exclude_path_globs.filter((g) => g !== glob),
      );
    },
...
export function RuleInspector({ rule, onSetHookSurface, onSetRuleCliSurface, onExclusionsChange, onClose, isMobile }: RuleInspectorProps) {
  const { draftExclusion, setDraftExclusion, sevColor, addExclusion, removeExclusion, toggleEvent, effectiveBehaviorSummary } =
      ...
      <Sheet open={!!rule} onOpenChange={(open) => !open && onClose()}>
        <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto bg-background p-6">
          <SheetHeader className="text-left mb-4">
            ...
            <SheetDescription className="text-muted-foreground font-sans">Operator configuration workspace</SheetDescription>
          </SheetHeader>
          {content}
        </SheetContent>
      </Sheet>
