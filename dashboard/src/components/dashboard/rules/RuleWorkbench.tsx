...
function useRuleMetadataWithChanges(config: SlopgateConfig, savedConfig: SlopgateConfig, fireCounts: Map<string, number>) {
  return useMemo(() => {
...
export const RuleWorkbench = memo(function RuleWorkbench({ fireCounts }: RuleWorkbenchProps) {
  const { config, savedConfig, setRuleHookSurface, setRuleCliSurface, setExclusions } = useRulesConfig();
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-stretch">
        <div className="flex-1 min-w-0">
          <RuleList allRules={allRules} selectedRuleId={selectedRuleId} onSelectRule={handleSelectRule} />
        </div>
        {selectedRule && (
          <RuleInspector
            rule={selectedRule}
