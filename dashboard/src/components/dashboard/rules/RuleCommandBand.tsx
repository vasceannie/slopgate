...
const TopBarStatus = memo(function TopBarStatus({
	apiAvailable,
	pendingCount,
	showPendingPanel,
		...
		<div className="flex items-center gap-3">
			<Badge
				...
				className={cn(
					...
					apiAvailable
						? "bg-signal-allow/10 text-signal-allow border-signal-allow/25"
...
const PendingChangesPanel = memo(function PendingChangesPanel({
	skipPathsChanged,
	pendingList,
	...
	return (
		<div className="p-3 border border-signal-ask/30 bg-signal-ask/5 rounded-md space-y-2 transition-all animate-in fade-in slide-in-from-top-2 duration-200 ease-out-quart">
			<div className="text-[0.875rem] font-semibold text-signal-ask">
...
const ExclusionBadgeList = memo(function ExclusionBadgeList({
	paths,
	onRemove,
}: ExclusionBadgeListProps) {
	if (paths.length === 0) {
		return (
			<span className="text-[0.875rem] text-muted-foreground italic">
				No global exclusions configured.
			</span>
		);
	}
	return (
...
const OperationalStatusPanel = memo(function OperationalStatusPanel({
	totals,
	...
	return (
		<div className="py-2.5 border-y border-border">
			<div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[0.875rem] text-muted-foreground mb-2">
				...
				<span>•</span>
				<span>Hook: <strong className="text-signal-allow">{totals.hookEnabled} on</strong> / <strong className="text-muted-foreground/80">{totals.hookOff} off</strong></span>
				<span>•</span>
				<span>CLI: <strong className="text-primary">{totals.cliEnabled} on</strong> / <strong className="text-muted-foreground/80">{totals.cliOff} off</strong></span>
				<span>•</span>
				<span>Active (recently firing): <strong className="text-signal-ask">{totals.activeFired}</strong></span>
			</div>
...
export function RuleCommandBand({ allRules }: RuleCommandBandProps) {
	...
	return (
		<div className="space-y-4 font-sans text-[1rem]">
			...
			/>
			{pendingCount > 0 && showPendingPanel && (
				<PendingChangesPanel
					skipPathsChanged={skipPathsChanged}
