...
export function AsyncJobs({ passCount, failCount, byCommand }: Props) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xs text-muted-foreground uppercase tracking-wider px-1">Async Jobs / Quality Follow-up</h3>

      <div className="grid grid-cols-2 gap-3">
        <div className="border border-border rounded-md bg-card/30 p-2">
          <h4 className="text-[10px] text-muted-foreground uppercase mb-1 text-center">Pass / Fail</h4>
        ...
        <div className="border border-border rounded-md bg-card/30 p-2">
          <h4 className="text-[10px] text-muted-foreground uppercase mb-1 text-center">Median Runtime (ms)</h4>
          <div className="h-[150px]">
            <ResponsiveBar
              data={runtimeData}
              keys={["runtime"]}
      ...
      <div className="border border-border rounded-md bg-card/30 p-3">
        <h4 className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2">Noisy Commands (Highest Failure Rate)</h4>
        <div className="space-y-1">
            ...
            <div key={c.command}>
              ...
              >
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-signal-deny">{((c.fail / c.total) * 100).toFixed(0)}% fail</span>
          ...
          {noisy.length === 0 && <div className="text-xs text-muted-foreground">No failing commands</div>}
        </div>
      </div>
    </div>
  );
