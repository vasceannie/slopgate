...
export function AsyncJobs({ passCount, failCount, byCommand }: Props) {
  ...
  const passFailTitle = hasJobs ? `${passCount} passed, ${failCount} failed` : "No async jobs ran in this window";
  ...
  const runtimeData = [...byCommand]
    ...
    .map((c) => {
      ...
      const percent = Math.min(RUNTIME_BAR_MAX_WIDTH_PERCENT, Math.max(0, (runtimeVal / maxRuntime) * RUNTIME_BAR_MAX_WIDTH_PERCENT));
      return {
        command: c.command,
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xs text-muted-foreground uppercase tracking-wider px-1">Async quality checks</h3>
        ...
        <>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="border border-border rounded-md bg-card/30 p-3">
              <h4 className="text-[11px] text-muted-foreground uppercase mb-2 text-center font-semibold tracking-wider">Pass / Fail</h4>
                ...
                <div className="flex h-full items-center justify-center gap-4">
                  ...
                  >
                    <div className="grid h-14 w-14 place-items-center rounded-full bg-card text-center">
                      <span className="font-mono text-xs font-semibold text-muted-foreground/85">{passFailLabel}</span>
                    </div>
                  </div>
                ...
                {runtimeData.map((row) => (
                  ...
                  >
                    ...
                    <div className="h-1.5 rounded-sm bg-muted/40 overflow-hidden">
                      <div className="h-full rounded-sm bg-primary transition-all duration-300" style={{ width: row.widthPercent }} />
                    </div>
                    <span className="text-right font-mono text-foreground">{row.runtime}</span>
                  </div>
                ))}
                {runtimeData.length === 0 && (
                  <div className="px-4 py-8 text-center text-xs text-muted-foreground">No async jobs ran in this window</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="border border-border rounded-md bg-card/30 p-3">
            ...
            <div className="space-y-1">
                ...
                <div key={c.command}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <span className="text-signal-deny font-medium">{(failureRate(c) * 100).toFixed(0)}% fail</span>
              ...
              {noisy.length === 0 && (
                <div className="px-2 py-3 text-center text-xs text-muted-foreground">No async command failures detected</div>
              )}
            </div>
          </div>
        </>
