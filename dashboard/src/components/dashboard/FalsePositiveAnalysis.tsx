import { AlertTriangle, Eye, Flag, Info, ShieldCheck, ShieldX } from "lucide-react";
...
export function FalsePositiveAnalysis({ rules, results }: Props) {
  ...
  const signals = useMemo(() => computeCalibrationSignals(rules, results), [rules, results]);
  ...
  const { needsReviewCount, repeatFireRulesCount, persistentRulesCount, highConfidenceCount, mediumConfidenceCount, lowConfidenceCount } =
    useMemo(() => {
      return {
        needsReviewCount: signals.filter((s) => s.isAdvisorySuspect || s.isRuntimeErrorSuspect || s.isVariableSuspect).length,
  ...
  const top5Rules = useMemo(() => {
    return activeSignals
      .map((s) => ({
        rule_id: s.rule_id,
        score: tab === "advisory" ? s.advisoryPressure : tab === "error" ? s.runtimeErrorPressure : s.decisionVariance,
      }))
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <SummaryCard icon={ShieldX} label="Needs Review" value={needsReviewCount} color="text-signal-ask" />
        <SummaryCard icon={ShieldCheck} label="Runtime/Repeat Rules" value={repeatFireRulesCount} color="text-signal-block" />
        <SummaryCard icon={AlertTriangle} label="Persistent Rules" value={persistentRulesCount} color="text-signal-error" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
        ...
        <div className="col-span-1 lg:col-span-3 min-h-[260px] border border-border rounded-md bg-card/30 p-3 flex flex-col justify-between">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground leading-normal font-sans">
              Tune policy enforcement by reviewing flagged rules. Select a lens to rank warning mismatches, repeated raw detector firings,
              or delivered findings that persisted across comparable hook runs, then click any row to inspect session evidence.
            </p>

            {/* Confidence Distribution */}
            <div className="pt-2 border-t border-border/30">
              <div className="text-xs font-semibold text-foreground">Evidence Confidence Distribution</div>
              <div className="flex gap-4 mt-1">
                <div className="flex items-center gap-1.5 text-xs">
                  <span className="w-1.5 h-1.5 rounded-full bg-signal-block" />
                  <span className="text-muted-foreground">High:</span>
                  <span className="font-mono text-foreground font-semibold">{highConfidenceCount}</span>
                </div>
                <div className="flex items-center gap-1.5 text-xs">
                  <span className="w-1.5 h-1.5 rounded-full bg-signal-ask" />
                  <span className="text-muted-foreground">Medium:</span>
                  <span className="font-mono text-foreground font-semibold">{mediumConfidenceCount}</span>
                </div>
                <div className="flex items-center gap-1.5 text-xs">
                  <span className="w-1.5 h-1.5 rounded-full bg-muted-foreground/50" />
                  <span className="text-muted-foreground">Low:</span>
                  <span className="font-mono text-foreground font-semibold">{lowConfidenceCount}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-border/30 mt-2">
            <div className="text-xs font-semibold text-foreground mb-1.5">Top Rules under {lensName(tab)} Lens</div>
            {top5Rules.length > 0 ? (
              <div className="space-y-1.5">
                {top5Rules.map((row) => (
                  <div key={row.rule_id} className="grid grid-cols-[120px_minmax(0,1fr)_30px] items-center gap-2 text-xs">
                    ...
                    <div className="h-2 rounded-sm bg-muted/40">
                      <div
                        className={cn(
                          ...
                          tab === "advisory" ? "bg-signal-ask" : tab === "error" ? "bg-signal-block" : "bg-signal-error",
                        )}
                        style={{
                          width: `${Math.max(1, (row.score / maxBarScore) * 100)}%`,
                        }}
                      />
                    </div>
                    <span className="text-right font-mono text-foreground font-bold">{row.score}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-xs text-muted-foreground italic">No suspect rules found for the active lens.</div>
            )}
          </div>
        </div>
        ...
        <div className="col-span-1 lg:col-span-2 min-h-[260px]">
          {selectedSignal ? (
            <div
              key={selectedSignal.rule_id}
              className="flex flex-col h-full bg-card border border-border rounded-md p-3 text-xs space-y-2 animate-fade-in"
            >
              <div className="flex justify-between items-start border-b border-border pb-2">
                <div className="min-w-0 flex-1 pr-2">
                  ...
                  <div
                    ...
                    title={getRuleDescription(selectedSignal.rule_id) || undefined}
                  >
              ...
              <div className="grid grid-cols-3 gap-2 border-b border-border/50 pb-2">
                <div className="text-center p-1.5 bg-muted/20 rounded">
                  ...
                  <div className="font-mono text-sm font-bold text-signal-ask">{selectedSignal.advisoryPressure}%</div>
                </div>
                <div className="text-center p-1.5 bg-muted/20 rounded">
                  <div className="text-xs font-semibold text-muted-foreground">Runtime score</div>
                  <div className="font-mono text-sm font-bold text-signal-block">{selectedSignal.runtimeErrorPressure}</div>
                </div>
                <div className="text-center p-1.5 bg-muted/20 rounded">
                  <div className="text-xs font-semibold text-muted-foreground">Persist score</div>
                  <div className="font-mono text-sm font-bold text-signal-error">{selectedSignal.decisionVariance}</div>
                </div>
              </div>

              <div className="space-y-1.5 flex-1 min-h-0 overflow-y-auto pr-1">
                <div className="text-xs font-semibold text-foreground">Evidence Details</div>
                ...
                <div className="border-t border-border/30 pt-1.5 mt-1">
                  <div className="text-xs font-semibold text-foreground">Decision Mix</div>
                ...
                {selectedSignal.recentExampleMessage && (
                  <div className="border-t border-border/30 pt-1.5 mt-1">
                    <div className="text-xs font-semibold text-foreground">Recent Finding Message</div>
                ...
                {selectedSignal.recentExampleError && (
                  <div className="border-t border-border/30 pt-1.5 mt-1">
                    <div className="text-xs font-semibold text-foreground">Recent Error Message</div>
                    <div className="bg-muted/30 border border-border/50 rounded p-1.5 mt-1 font-mono text-xs break-all leading-normal text-muted-foreground max-h-16 overflow-y-auto">
                      {selectedSignal.recentExampleError}
                    </div>
                  </div>
          ...
          <table className="w-full text-xs min-w-[800px]">
            ...
            <tbody>
              {active.length === 0 ? (
                ...
                active.map((s) => {
                  const currentScore =
                    tab === "advisory" ? s.advisoryPressure : tab === "error" ? s.runtimeErrorPressure : s.decisionVariance;
                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-2 font-medium" title={getRuleDescription(s.rule_id) || s.rule_id}>
                        <button
                          type="button"
                          onClick={(event) => {
                        ...
                        >
                          {selectedRuleId === s.rule_id && <span className="w-1.5 h-1.5 rounded-full bg-primary shrink-0" />}
                          {s.rule_id}
                        </button>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex flex-wrap gap-1">
                          {s.isAdvisorySuspect && (
                            <span className="text-xs px-2 py-0.5 rounded-md bg-signal-ask/15 text-signal-ask font-semibold">Advisory</span>
                          )}
                          ...
                          {s.isClean && (
                            <span className="text-xs px-2 py-0.5 rounded-md bg-muted text-muted-foreground font-semibold">Clean</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex gap-2 text-xs font-mono">
                          <span className="text-signal-block font-semibold" title="Blocked">
                            B:{s.blockCount}
                          </span>
                          <span className="text-signal-ask font-semibold" title="Warned">
                            W:{s.warnCount}
                          </span>
                          <span className="text-muted-foreground" title="Allowed">
                            A:{s.allowCount}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-2 text-right" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                        <FlagButton itemType="rule" itemId={s.rule_id} label={label} compact />
                      </td>
                    </tr>
                  );
                })
...
const SummaryCard = memo(function SummaryCard({
  ...
  return (
    <div className="flex items-center gap-3 px-3.5 py-3 rounded-lg border border-border bg-card/40 transition-colors hover:bg-card/60">
      ...
      <div>
        <div className="text-xl font-bold font-sans tracking-tight text-foreground leading-none mb-0.5">{value}</div>
        <div className="text-[11px] font-medium text-muted-foreground leading-none">{label}</div>
      </div>
    </div>
  );
});
