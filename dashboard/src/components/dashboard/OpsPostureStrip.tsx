...
export function OpsPostureStrip({
	asyncFailCount,
	config,
	harnessStatus,
}: OpsPostureStripProps) {
	...
	const partialPlatforms = platforms.filter(
		(p) => p.status === "partial" || p.status === "disabled" || p.status === "missing",
	);

	let posture: "Checking" | "Quiet" | "Degraded" | "Action Needed" = loading
		? "Checking"
		: "Quiet";
	...
	if (loading) {
		...
		if (asyncFailCount > 0 && (errorPlatforms.length > 0 || error)) {
			...
			statusMessage = error
				? `Failed to load harness status: ${error}`
				: `${errorPlatforms.length} platform harness error(s) detected.`;
		...
		partialPlatforms.length > 0 ||
		disabledRulesCount > 0 ||
		severityOverridesCount > 0
	) {
		posture = "Action Needed";
	...
	const harnessSummary = loading
		? "..."
		: hasHarnessData
			? `${installedPlatforms.length}/${platforms.length}`
			: "—";
			...
			<div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between w-full">
				...
				<div className="flex flex-wrap items-center gap-x-8 gap-y-3 py-3 border-y border-border lg:border-y-0 lg:border-x lg:px-8 lg:py-0 shrink-0">
						...
						<div className="flex items-center gap-1.5">
							...
							<span
								className={cn(
									...
									asyncFailCount > 0 ? "text-signal-deny" : "text-foreground",
								)}
							>
								{asyncFailCount}
							</span>
					...
					<div className="flex flex-col gap-0.5">
						...
						<div className="flex items-center gap-2">
							...
							<span className="text-base font-mono font-bold text-foreground">
								{disabledRulesCount + severityOverridesCount}
							</span>
					...
					<div className="flex flex-col gap-0.5">
						...
						<div className="flex items-center gap-1.5">
							...
							{!loading && !hasHarnessData && (
								<span className="text-[10px] text-muted-foreground font-medium">(unavailable)</span>
							)}
							{errorPlatforms.length > 0 && (
								<span className="text-[10px] text-signal-deny font-medium">
									({errorPlatforms.length} err)
								</span>
							)}
						</div>
					</div>
				...
				<div className="flex flex-col gap-1.5 lg:max-w-xs xl:max-w-md w-full lg:w-auto shrink-0">
					...
					<div className="flex items-center justify-between gap-3 bg-muted/40 border border-border/80 rounded px-3 py-2 font-mono text-xs text-foreground group relative">
						...
						>
							{copied ? (
								<Check className="w-4 h-4 text-signal-allow" />
							) : (
