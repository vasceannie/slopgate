...
import { RulesConfigContext, type RulesConfigContextValue } from "@/context/rulesConfigContext";
...
describe("RuleManager", () => {
  it("routes hook and CLI surfaces through the canonical rule row", () => {
    ...
    fireEvent.click(screen.getByRole("switch", { name: "PY-CODE-008 hook enablement" }));
    fireEvent.click(screen.getByRole("switch", { name: "PY-CODE-008 CLI enablement" }));
    fireEvent.click(screen.getByRole("switch", { name: "PY-CODE-008 CLI enablement" }));

    expect(value.setRuleHookSurface).toHaveBeenCalledWith("PY-CODE-008", {
      enabled: false,
    });
    expect(value.setRuleCliSurface).toHaveBeenCalledWith("PY-CODE-008", ["long-method"], { enabled: false });
  ...
  it("folds mapped CLI checks into their hook rule row", () => {
    ...
    expect(screen.queryByRole("button", { name: "long-method" })).not.toBeInTheDocument();
  });
  ...
  it("exposes hook rules backed by new batch lint collectors as dual-surface", () => {
    ...
    fireEvent.click(screen.getByRole("switch", { name: "PY-CODE-012 CLI enablement" }));

    expect(value.setRuleCliSurface).toHaveBeenCalledWith("PY-CODE-012", ["feature-envy"], { enabled: false });
    expect(screen.queryByRole("button", { name: "feature-envy" })).not.toBeInTheDocument();
  });
  ...
  it("exposes import hook rules as opt-in CLI counterparts", () => {
    ...
    fireEvent.click(screen.getByRole("switch", { name: "PY-IMPORT-002 CLI enablement" }));

    expect(value.setRuleCliSurface).toHaveBeenCalledWith("PY-IMPORT-002", ["import-alias"], { enabled: true });
    expect(screen.queryByRole("button", { name: "import-alias" })).not.toBeInTheDocument();
  });
  ...
  it("exposes boundary logging as an opt-in CLI counterpart", () => {
    ...
    fireEvent.click(screen.getByRole("switch", { name: "PY-LOG-002 CLI enablement" }));

    expect(value.setRuleCliSurface).toHaveBeenCalledWith("PY-LOG-002", ["boundary-logging"], { enabled: true });
    expect(screen.queryByRole("button", { name: "boundary-logging" })).not.toBeInTheDocument();
  });
  ...
  it("exposes pytest asyncio as an opt-in CLI counterpart", () => {
    ...
    fireEvent.click(screen.getByRole("switch", { name: "PY-TEST-005 CLI enablement" }));

    expect(value.setRuleCliSurface).toHaveBeenCalledWith("PY-TEST-005", ["pytest-asyncio-pattern"], { enabled: true });
    expect(screen.queryByRole("button", { name: "pytest-asyncio-pattern" })).not.toBeInTheDocument();
  });
  ...
  it("exposes LangGraph rules as opt-in CLI counterparts", () => {
    ...
    fireEvent.click(screen.getByRole("switch", { name: "LG-API-001 CLI enablement" }));

    expect(value.setRuleCliSurface).toHaveBeenCalledWith("LG-API-001", ["langgraph-deprecated-api"], { enabled: true });
    expect(screen.queryByRole("button", { name: "langgraph-deprecated-api" })).not.toBeInTheDocument();
  });
  ...
  it("labels unsupported surfaces instead of rendering an ambiguous dash", () => {
    ...
    expect(screen.getByRole("switch", { name: "CONFIG-001 hook enablement" })).toBeInTheDocument();
    expect(screen.getAllByText("config safety")[0]).toBeInTheDocument();
    expect(screen.queryByRole("switch", { name: "CONFIG-001 CLI enablement" })).not.toBeInTheDocument();
  });
  ...
  it("labels CLI-only rows as source lint", () => {
    ...
    expect(screen.getAllByText("source lint available").length).toBeGreaterThan(0);
    expect(screen.queryByRole("switch", { name: "long-test hook enablement" })).not.toBeInTheDocument();
  });
  ...
  it("exposes content regex rules as real hook and CLI surfaces", () => {
    ...
    expect(value.setRuleCliSurface).toHaveBeenCalledWith("CUSTOM-CONTENT-001", ["CUSTOM-CONTENT-001"], { enabled: true });
  });
