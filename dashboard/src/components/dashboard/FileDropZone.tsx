...
export function FileDropZone() {
	...
	const lastAcceptedTraceLabel = formatTraceTimestamp(
		sourceMeta.lastAcceptedStreamRecordAt,
	);
			...
			>
				...
				{loading ? (
					...
					<div className="flex flex-wrap items-center gap-2 text-xs">
						{streamState === "connecting" ? (
							<>
								...
								<span className="font-medium text-amber-300">
									{SOURCE_MESSAGES[sourceMode]}
								</span>
								<span className="rounded-full border border-amber-500/40 bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-semibold text-amber-300">
									CONNECTING
								</span>
							</>
							...
							<>
								...
								<span className="font-medium text-amber-300">
									Live stream interrupted — retrying with snapshot data
								</span>
								<span className="rounded-full border border-amber-500/40 bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-semibold text-amber-300">
									RETRYING
								</span>
						...
						{lastEventLabel && (
							<span className="text-muted-foreground">
								transport event {lastEventLabel}
							</span>
						)}
						{latestTraceLabel && (
							<span className="text-muted-foreground">
								dataset latest {latestTraceLabel}
							</span>
						)}
						{lastAcceptedTraceLabel && sourceMeta.acceptedStreamRecords > 0 && (
							<span className="text-muted-foreground">
								accepted trace {lastAcceptedTraceLabel}
							</span>
						)}
						{sourceMeta.rejectedStreamRecords > 0 &&
							sourceMeta.acceptedStreamRecords === 0 && (
								<span className="flex items-center gap-1 text-amber-300">
									...
									{sourceMeta.rejectedStreamRecords} stream record
									{sourceMeta.rejectedStreamRecords === 1 ? "" : "s"} rejected
								</span>
							)}
					</div>
				) : (
					<div className="flex items-center gap-2 text-xs text-muted-foreground">
						...
						{SOURCE_MESSAGES.mock} or{" "}
						<span className="text-primary underline">browse</span>
						<span className="text-muted-foreground/60 ml-1">
							(events, rules, results, subprocess)
						</span>
					</div>
				)}
			</button>
			...
			{status && status.rejected.length > 0 && (
				<div className="mt-1.5 space-y-0.5">
					{status.rejected.map((msg: string) => (
						<div
							key={msg}
							className="flex items-center gap-1.5 text-[10px] text-signal-block"
						>
