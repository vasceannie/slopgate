import { AlertOctagon, AlertTriangle, HelpCircle, Shield, SkipForward, XCircle } from "lucide-react";
      ...
      {sourceStatus?.warning && (
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
          ...
          <span className="text-amber-200/70">
            accepted {sourceStatus.meta.totalRecords.toLocaleString()} records · dataset latest{" "}
            {sourceStatus.meta.latestDataAt ?? "unknown"}
          </span>
        </div>
      )}
