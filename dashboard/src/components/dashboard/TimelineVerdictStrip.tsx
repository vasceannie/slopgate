...
export function TimelineVerdictStrip({ entry }: TimelineVerdictStripProps) {
  ...
  const isAdvisory = entry.decision !== "block" && entry.decision !== "deny" && entry.decision !== "allow" && entry.decision !== undefined;

  return (
    <div className="bg-muted/10 border border-border/40 rounded-md p-3 mb-3 text-xs select-none transition-all duration-300 hover:bg-muted/15 hover:border-border/60">
      <div className="flex items-start gap-2.5">
        {isBlocked ? (
          <AlertOctagon className="w-5 h-5 text-signal-deny shrink-0 mt-0.5 transition-transform duration-200 hover:scale-110" />
        ) : isAdvisory ? (
          <ShieldAlert className="w-5 h-5 text-signal-ask shrink-0 mt-0.5 transition-transform duration-200 hover:scale-110" />
        ) : (
          <CheckCircle2 className="w-5 h-5 text-signal-allow shrink-0 mt-0.5 transition-transform duration-200 hover:scale-110" />
        )}
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            {entry.decision && (
              <span className={cn("px-1.5 py-0.5 rounded border text-[10px] uppercase font-bold", DECISION_BADGE_STYLE[entry.decision])}>
                {entry.decision}
              </span>
            )}
          </div>
          <div className="text-muted-foreground text-[11px] mt-1 space-y-1">
            ...
            {entry.candidate_paths && entry.candidate_paths.length > 0 && (
              <p className="truncate" title={entry.candidate_paths.join(", ")}>
                ...
                <span className="text-foreground font-mono">{entry.candidate_paths.join(", ")}</span>
              </p>
            )}
          </div>
        </div>
