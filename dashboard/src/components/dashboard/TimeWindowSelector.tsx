...
export function TimeWindowSelector({ filters, onChange }: Props) {
  ...
  const setTimeWindow = useCallback(
    (timeWindow: TimeWindow) => {
      ...
      if (sourceMode !== "uploaded" && (loadedHours < hours || sourceMeta.snapshotError)) {
        void refreshSnapshot(hours);
      }
    },
    [filters, onChange, refreshSnapshot, sourceMeta.snapshotError, sourceMeta.snapshotLookbackHours, sourceMode],
  );
    ...
    <div className="flex items-center gap-6 py-3 border-b border-border bg-card/50">
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className={cn(
            "px-3 py-1 text-xs rounded-sm transition-colors",
            filters.platforms.length === 0 ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-muted",
          )}
        >
          All
        </button>
