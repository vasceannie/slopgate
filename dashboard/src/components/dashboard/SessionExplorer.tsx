...
import { DECISION_BADGE_STYLE, PLATFORM_BADGE_STYLE, SEVERITY_TEXT_STYLE } from "@/lib/chartTheme";
...
import { calculateScrollAdjustment, determineAnchor } from "./sessionExplorerAnchoring";
...
function matchesDateRange(session: SessionData, dateRange: DateRangeFilter): boolean {
...
function matchesSearchQuery(session: SessionData, query: string): boolean {
  ...
  const hasRuleId =
    ...
    session.results.some((r) => r.findings.some((f) => f.rule_id?.toLowerCase().includes(q)));
  if (hasRuleId) return true;

  // Candidate paths
  const cause = primarySessionCause(session);
  const causePaths = cause.paths || [];
  const eventCandidatePaths = session.events.flatMap((e) => e.candidate_paths ?? []);
  ...
  // Command text
  const eventCommands = session.events.map((e) => e.command).filter((c): c is string => !!c);
  const resultCommands = session.results.map((r) => r.command).filter((c): c is string => !!c);
  const subprocessCommands = session.subprocesses.map((s) => s.command).filter((c): c is string => !!c);
  const allCommands = [...eventCommands, ...resultCommands, ...subprocessCommands];
...
function matchesSessionFilters(
  ...
  return (
    (selectedOutcomes.size === 0 || selectedOutcomes.has(session.finalOutcome)) &&
    (selectedPlatforms.size === 0 || (session.platforms ?? [session.platform]).some((platform) => selectedPlatforms.has(platform))) &&
    (selectedLanguages.size === 0 || session.languages.some((language) => selectedLanguages.has(language))) &&
...
export function SessionExplorer({ sessions }: Props) {
  ...
  const [selectedOutcomes, setSelectedOutcomes] = useState<Set<Decision>>(() => new Set());
  ...
  const perPage = 15;
  const outcomes = useMemo(() => [...new Set(sessions.map((session) => session.finalOutcome))].sort(), [sessions]);
  const platforms = useMemo(() => [...new Set(sessions.flatMap((session) => session.platforms ?? [session.platform]))].sort(), [sessions]);
  const languages = useMemo(() => [...new Set(sessions.flatMap((session) => session.languages))].sort(), [sessions]);
  const filteredSessions = useMemo(
    () =>
      sessions.filter((session) =>
        matchesSessionFilters(session, selectedOutcomes, selectedPlatforms, selectedLanguages, dateRange, searchQuery),
      ),
    [sessions, selectedOutcomes, selectedPlatforms, selectedLanguages, dateRange, searchQuery],
  );
  const paginated = filteredSessions.slice(page * perPage, (page + 1) * perPage);
  ...
  useLayoutEffect(() => {
    ...
    if (anchorRef.current) {
      ...
      const adj = calculateScrollAdjustment(id, oldTop, tableBodyRef.current, window.scrollY);
      if (adj !== 0) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-2 px-1">
        <h3 className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Session & Tool Explorer</h3>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 border-b border-border bg-background/25 px-3 py-2 text-[10px]">
          <FilterGroup label="Outcome">
            <MultiSelectMenu
              ...
              onToggle={(outcome) => {
                isUserActionRef.current = true;
                setSelectedOutcomes((current) => toggleSetValue(current, outcome));
                resetSessionPaging();
              }}
            />
          </FilterGroup>
          ...
          <FilterGroup label="Language">
            <MultiSelectMenu
              ...
              onToggle={(language) => {
                isUserActionRef.current = true;
                setSelectedLanguages((current) => toggleSetValue(current, language));
                resetSessionPaging();
              }}
            />
          </FilterGroup>
        ...
        <div className="flex items-center justify-between px-3 py-2 border-t border-border text-[10px] text-muted-foreground">
          ...
          <div className="flex gap-2">
            ...
            <span className="font-mono tabular-nums">
              {page + 1}/{Math.max(1, Math.ceil(filteredSessions.length / perPage))}
            </span>
            <button
              type="button"
...
const SessionRow = memo(function SessionRow({
  ...
  const mirrorOnlySessions = mirrorSessions.filter((session) => !childSessionIds.has(session.id));
  const rawSessionIds = s.rawSessionIds ?? [s.id];
  const displayName = sessionDisplayName(s);
  const hasTitle = Boolean(s.title?.trim());
  const childMirrorCount = childSessions.filter((session) => session.lineageRole === "child_mirror").length;
      ...
      >
        ...
        <td className="px-3 py-2 font-mono align-middle whitespace-nowrap overflow-hidden">
          <span className="flex items-center gap-1 min-w-0" title={sessionTitleText(s)}>
            <span
              className={cn("truncate", hasTitle ? "font-sans font-medium text-foreground" : "font-mono")}
        ...
        <td className="px-3 py-2 align-middle truncate whitespace-nowrap overflow-hidden" title={cause.message || ""}>
          {cause.decision === "allow" ? (
            <span className="text-signal-allow font-medium text-[10px] truncate block">Clean allow</span>
          ) : (
            <span className="flex items-center gap-1 text-[10px] truncate w-full">
              {cause.severity && <span className={cn("font-bold shrink-0", SEVERITY_TEXT_STYLE[cause.severity])}>[{cause.severity}]</span>}
              {cause.ruleId && <strong className="text-foreground shrink-0 truncate max-w-[100px]">{cause.ruleId}: </strong>}
              <span className="text-muted-foreground truncate min-w-0">{cause.message}</span>
            </span>
          )}
        </td>
        <td className="px-3 py-2 align-middle whitespace-nowrap overflow-hidden">
        ...
        <td className="px-3 py-2 align-middle whitespace-nowrap overflow-hidden">
          <div className="flex items-center gap-1 select-none">
            {activity.lastTool ? (
              ...
              <span className="text-muted-foreground text-[10px] italic inline-block w-20 text-center shrink-0">None</span>
            )}
                  ...
                  {firstPath ? (
                    <span
                      className={cn(
                        ...
                        causePaths.includes(firstPath) && "border border-primary/20 bg-primary/5 text-primary",
                      )}
                      title={firstPath}
                    >
                      {firstPath.split("/").pop()}
                    ...
                    <span className="text-muted-foreground text-[10px] italic inline-block w-32 shrink-0">None</span>
                  )}
                  {extraPathsCount > 0 ? (
                    <span
                      className="text-muted-foreground text-[10px] font-semibold tabular-nums shrink-0 w-6 text-right"
        ...
        <td className="px-3 py-2 align-middle whitespace-nowrap overflow-hidden">
          <FlagButton itemType="session" itemId={s.id} label={`${displayName} (${s.platform}, ${s.finalOutcome})`} compact />
        </td>
      </tr>
      {isExpanded && (
        <tr>
              ...
              {(lineageCount > 0 || (s.rawSessionIds ?? []).length > 1) && (
                <div className="border-b border-border bg-background/40 px-4 py-3 text-[10px] text-muted-foreground">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-semibold uppercase tracking-wide text-foreground">Lineage</span>
                    <span className="rounded border border-border/40 bg-muted/20 px-1.5 py-0.5">{s.lineageConfidence ?? "none"}</span>
                    ...
                    {childOnlyCount > 0 && (
                      <span className="rounded border border-border/40 bg-muted/20 px-1.5 py-0.5">{childOnlyCount} child</span>
                    )}
                    {childMirrorCount > 0 && (
                      <span className="rounded border border-border/40 bg-muted/20 px-1.5 py-0.5">{childMirrorCount} child + mirror</span>
                    )}
                    {mirrorOnlyCount > 0 && (
                      <span className="rounded border border-border/40 bg-muted/20 px-1.5 py-0.5">{mirrorOnlyCount} mirror</span>
                    )}
                  </div>
                </div>
              )}
