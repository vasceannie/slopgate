import { AlertTriangle, Check, CheckCircle2, Code2, Loader2, Paperclip, Plus, RefreshCw, Send, Sparkles, Terminal, X } from "lucide-react";
...
export function RuleIterationModal({ isOpen, onClose, initialProjectName = "", projects, onRuleCreated }: RuleIterationModalProps) {
  // Form State
  ...
  const startHermesSession = () => {
    ...
    setTimeout(() => {
      setLogs((prev) => [...prev, { type: "system", text: "Handshake established. Mounting isolated workspace sandbox..." }]);
    }, 600);
  ...
  const streamRuleDetails = () => {
    const steps: Array<{ log: TerminalLog; diffChunk?: string; delay: number }> = [
      ...
      },
      {
        log: {
          type: "thought",
          text: `Scanning code references: ${fileRefs.length > 0 ? fileRefs.join(", ") : "general project scope"}...`,
        },
        delay: 900,
      },
      {
  ...
  const handleSendIteration = () => {
    ...
    setTimeout(() => {
      setLogs((prev) => [...prev, { type: "command", text: "slopgate rule compile --check-tests" }]);
    }, 1200);
      ...
      <DialogContent className="max-w-4xl h-[650px] bg-background border border-border p-0 flex flex-col overflow-hidden text-foreground">
        ...
        {modalState === "setup" && (
          <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-[1.2fr_0.8fr] gap-8 font-sans">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Select value={selectedProject} onValueChange={setSelectedProject}>
                    <SelectTrigger
                      id="project-select"
                      className="h-8 text-xs bg-muted/20 border-border/80 focus:ring-primary/40 focus:border-primary font-mono"
                    >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1.5">
                    {fileRefs.map((ref, idx) => (
                      <Badge
                        key={ref}
                        variant="secondary"
                        className="px-2 py-0.5 text-[9px] font-mono gap-1 border border-border/40 hover:bg-muted text-foreground"
                      >
                        {ref}
                        <button
                          type="button"
                          onClick={() => handleRemoveFileRef(idx)}
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-1.5">
                <Label className="text-[10px] font-mono uppercase text-muted-foreground">Attachments (Logs, Lint Outputs)</Label>
                ...
                {attachments.length > 0 && (
                  <div className="space-y-1 pt-1.5">
                    {attachments.map((name, idx) => (
                      <div
                        key={name}
                        className="flex items-center justify-between p-1.5 bg-muted/20 border border-border/30 rounded text-[10px] font-mono"
                      >
              ...
              <Card className="border border-border bg-card/25 p-3.5 rounded-lg space-y-2.5">
                <h5 className="text-[10px] font-semibold uppercase font-mono tracking-wider text-primary">Target Context Information</h5>
                ...
                <div className="text-[10px] text-muted-foreground leading-normal font-sans border-t border-border/40 pt-2.5">
                  Hermes will analyze code paths, imports, and attachments to design a minimal rule syntax conforming to Slopgate parser
                  standards.
                </div>
              </Card>
            </div>
          ...
          <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-2 border-t border-border bg-card/10">
            ...
            <div className="border-r border-border/60 flex flex-col overflow-hidden bg-black/45">
              ...
              <div className="p-3 border-t border-border bg-card/30 flex gap-2 items-center">
                <Input
                  placeholder={
                    modalState === "confirming" ? "Iterate on rule (e.g. 'exclude test files')" : "WebSocket session establishing..."
                  }
                  value={userInput}
            ...
            <div className="flex flex-col overflow-hidden bg-zinc-950">
              <div className="p-2 border-b border-border bg-card/60 flex items-center justify-between text-[10px] uppercase font-mono text-muted-foreground">
                ...
                <span className="text-[9px] text-muted-foreground normal-case font-mono">{targetProject.projectName}/.slopgate.toml</span>
              </div>
        ...
        {modalState === "applied" && (
          <div className="flex-1 p-8 flex flex-col items-center justify-center text-center space-y-4 max-w-xl mx-auto font-sans">
            ...
            <div className="space-y-1.5">
              <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider font-sans">Rule Appended Successfully!</h3>
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                The project-specific rule{" "}
                <code className="font-mono bg-muted px-1.5 py-0.5 rounded border border-border/40 text-foreground">
                  PY-{ruleName.toUpperCase().replace(/\s+/g, "_")}
                </code>{" "}
                has been compiled and appended to the workspace context.
              </p>
            </div>
            ...
            <Card className="border border-border bg-card/30 p-4 w-full text-left font-mono text-[10.5px] leading-relaxed space-y-1.5">
              ...
              <div>
                <span className="text-muted-foreground">Project:</span> <span className="text-foreground">{targetProject.projectName}</span>
              </div>
              <div>
