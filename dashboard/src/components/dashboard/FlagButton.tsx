    ...
    >
      ...
      <div>
        ...
        <div className="flex gap-1">
          {(Object.entries(FLAG_TARGET_LABELS) as [FlagTarget, (typeof FLAG_TARGET_LABELS)[FlagTarget]][]).map(
            ([key, { label, color }]) => (
              <button
                type="button"
                key={key}
        ...
        <div className="space-y-1.5 border-t border-border pt-2">
          ...
          {existingFlags.map((f) => (
            ...
            >
              ...
              <button type="button" onClick={() => removeFlag(f.id)} className="text-muted-foreground hover:text-signal-block">
                <X className="w-3 h-3" />
              </button>
            </div>
          ))}
