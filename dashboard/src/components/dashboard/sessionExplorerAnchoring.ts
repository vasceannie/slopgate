...
export function findFirstVisibleRow(tableBody: HTMLTableSectionElement | null): HTMLElement | null {
...
export function determineAnchor({
  ...
  if (expanded && tableBody) {
    anchorEl = tableBody.querySelector(`tr[data-session-id="${expanded}"]`) as HTMLElement | null;
  }
...
export function calculateScrollAdjustment(
  ...
  const newEl = tableBody.querySelector(`tr[data-session-id="${anchorId}"]`) as HTMLElement | null;
  if (!newEl) return 0;
