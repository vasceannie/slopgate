...
function textContentMatch(pattern: RegExp) {
  return (_content: string, node: Element | null) => {
    const hasText = (element: Element | null) => pattern.test(element?.textContent || "");
    return hasText(node) && Array.from(node?.children ?? []).every((child) => !hasText(child));
  };
}
...
describe("SessionTimeline", () => {
  ...
  it("renders unioned parent child and mirror trace fixture data in the component UI", () => {
    ...
    const { sessions } = buildTraceSessionIndexes(traceEvents, [], traceResults, []);
    ...
    expect(screen.queryByText(/source child:/)).not.toBeInTheDocument();
    expect(screen.queryByText(/source mirror:/)).not.toBeInTheDocument();
    expect(screen.getByText("Child mirror denial should control parent outcome.")).toBeInTheDocument();
  ...
  it("shows edit diffs and raw tool input in expanded hook drilldown", () => {
    ...
    expect(screen.getAllByText("Patch / focused diff:")[0]).toBeInTheDocument();
    expect(screen.getAllByText("Tool input:")[0]).toBeInTheDocument();
    expect(screen.getAllByText(/\/repo\/src\/slopgate\/engine\/_evaluation\.py/).length).toBeGreaterThan(0);
    expect(screen.getAllByText(/-return \{'tool_input': None\}/)[0]).toBeInTheDocument();
    expect(screen.getAllByText(/\+return \{'tool_input': ctx\.tool_input\}/)[0]).toBeInTheDocument();
  ...
  it("carries pre-tool code into later post-tool rows", () => {
      ...
      events: [
        {
          ...
          candidate_paths: ["/repo/dashboard/src/components/SessionTimeline.tsx"],
          languages: ["typescript"],
          tool_input: {
            patchText: [
              "*** Begin Patch",
    ...
    expect(screen.getAllByText("Patch / focused diff:")[0]).toBeInTheDocument();
    expect(
      screen.getAllByText("diff --git a/dashboard/src/components/SessionTimeline.tsx b/dashboard/src/components/SessionTimeline.tsx")[0],
    ).toBeInTheDocument();
    expect(screen.getAllByText("--- a/dashboard/src/components/SessionTimeline.tsx")[0]).toBeInTheDocument();
    expect(screen.getAllByText("+++ b/dashboard/src/components/SessionTimeline.tsx")[0]).toBeInTheDocument();
    expect(screen.getAllByText(/\+new/).length).toBeGreaterThan(0);
    expect(screen.getAllByText("Tool input:")[0]).toBeInTheDocument();
    expect(screen.getAllByText("patchText")[0]).toBeInTheDocument();
    expect(screen.getAllByText(/Captured apply_patch body/)[0]).toBeInTheDocument();
    ...
    expect(screen.getAllByText(/\*\*\* Begin Patch/)[0]).toBeInTheDocument();

    fireEvent.click(screen.getAllByRole("button", { name: /Tool input raw view/i })[0]);
    expect(screen.getAllByText('"patchText"')[0]).toBeInTheDocument();
  });
  ...
  it("beautifies generic tool input aliases while keeping raw JSON available", () => {
    ...
    expect(screen.getAllByText(/uv run pytest tests\/test_dashboard\.py -q/).length).toBeGreaterThan(0);
  ...
  it("preserves direct url-only tool payloads in pretty and raw views", () => {
    ...
    expect(screen.getAllByText("https://docs.example.test/reference").length).toBeGreaterThan(0);
  ...
  it("explains when historical apply_patch traces lack captured body content", () => {
      ...
      events: [
        {
          ...
          tool_name: "apply_patch",
          candidate_paths: ["/repo/dashboard/scripts/build_standalone/__init__.pyi"],
          languages: ["python"],
        },
      ],
      results: [
          ...
          findings: [
            {
              ...
              severity: "LOW",
              decision: "context",
              message: "/repo/dashboard/scripts/build_standalone/__init__.pyi imports 6 names from coercion.",
    ...
    expect(screen.getAllByText("Tool input unavailable")[0]).toBeInTheDocument();
    expect(screen.getAllByText(/Tool call body was not captured/)[0]).toBeInTheDocument();
    expect(screen.getAllByText("Grouped finding(s):")[0]).toBeInTheDocument();
  ...
  it("renders finding additional context as formatted evidence code", () => {
          ...
          findings: [
            {
              ...
              additional_context: '{"policy":"deterministic","nested":{"language":"json"}}',
            },
            {
              rule_id: "PY-CODE-012",
              severity: "LOW",
    ...
    expect(screen.getAllByText(textContentMatch(/"policy": "deterministic"/)).length).toBeGreaterThan(0);
    expect(screen.getAllByText(textContentMatch(/def choose_policy\(value\):/)).length).toBeGreaterThan(0);
  });
  ...
  it("formats Claude hookSpecificOutput context instead of raw JSON", () => {
      ...
      results: [
        {
          ...
          output: {
            hookSpecificOutput: {
              ...
              additionalContext: ["[PY-CODE-012 | LOW] first advisory", "[PY-IMPORT-001 | LOW] second advisory"].join("\n"),
            },
    ...
    expect(screen.getAllByText("Permission decision")[0]).toBeInTheDocument();
    ...
    expect(
      screen.getAllByText((_content, node) => {
        const hasText = (n: Element | null) => /\[PY-CODE-012 \| LOW\] first advisory/.test(n.textContent || "");
        return hasText(node) && Array.from(node?.children ?? []).every((c) => !hasText(c));
      })[0],
    ).toBeInTheDocument();
    expect(
      screen.getAllByText((_content, node) => {
        const hasText = (n: Element | null) => /\[PY-IMPORT-001 \| LOW\] second advisory/.test(n.textContent || "");
        return hasText(node) && Array.from(node?.children ?? []).every((c) => !hasText(c));
      })[0],
    ).toBeInTheDocument();
    expect(
      screen.getAllByText((_content, node) => {
        const hasText = (n: Element | null) => /\[PY-TEST-005 \| LOW\] root advisory/.test(n.textContent || "");
        return hasText(node) && Array.from(node?.children ?? []).every((c) => !hasText(c));
      })[0],
    ).toBeInTheDocument();

    fireEvent.click(screen.getAllByRole("button", { name: /Output raw view/i })[0]);
    expect(screen.getAllByText('"hookSpecificOutput"')[0]).toBeInTheDocument();
  });
  ...
  it("formats Codex and OpenCode output keys", () => {
    ...
    fireEvent.click(screen.getAllByRole("button", { name: /result: allow/i })[0]);
    expect(screen.getAllByText("Continue")[0]).toBeInTheDocument();
    expect(screen.getAllByText("Stop reason")[0]).toBeInTheDocument();

    fireEvent.click(screen.getAllByRole("button", { name: /result: allow/i })[1]);
  ...
  it("closes menus and flag panels on Escape key and returns focus to their triggers", () => {
    ...
    // Panel should close
    expect(screen.queryByText("Flag for Investigation")).not.toBeInTheDocument();
  });
});
...
describe("SessionExplorer", () => {
    ...
    const sessions: SessionData[] = [
      {
        ...
        rawSessionIds: ["parent-session-alpha", "child-session-beta", "mirror-session-gamma"],
  ...
  it("counts child_mirror linked sessions once in lineage details", () => {
    ...
    expect(screen.getByText("2 grouped sessions")).toBeInTheDocument();
    expect(screen.getByText("1 child + mirror")).toBeInTheDocument();
    expect(screen.queryByText("mirror: child-mirror-session-beta")).not.toBeInTheDocument();
    expect(screen.queryByText("child + mirror: child-mirror-session-beta")).not.toBeInTheDocument();
    expect(screen.queryByText("unclassified: child-mirror-session-beta")).not.toBeInTheDocument();
    expect(screen.queryByText("raw: child-mirror-session-beta")).not.toBeInTheDocument();
  });
});
