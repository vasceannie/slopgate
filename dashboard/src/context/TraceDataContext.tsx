...
function latestTraceTimestamp(data: TraceData): string | null {
  return latestTimestamp([...data.events, ...data.rules, ...data.results, ...data.subprocesses]);
}
...
function appendBoundedUnique<T extends object>(items: T[], item: T, knownKeys: Set<string>): T[] {
...
export function TraceDataProvider({ children }: { children: ReactNode }) {
  ...
  const [snapshotSummary, setSnapshotSummary] = useState<TraceSnapshotSummary | null>(null);
  ...
  const appendRecord = useCallback((obj: Record<string, unknown>): AppendOutcome => {
    ...
    if (accepted.type === "event") {
      setData((prev: TraceData) => {
        const events = appendBoundedUnique(prev.events, accepted.record, recordKeysRef.current.events);
    ...
    if (accepted.type === "rule") {
      setData((prev: TraceData) => {
        const rules = appendBoundedUnique(prev.rules, accepted.record, recordKeysRef.current.rules);
    ...
    if (accepted.type === "result") {
      setData((prev: TraceData) => {
        const results = appendBoundedUnique(prev.results, accepted.record, recordKeysRef.current.results);
    ...
    if (accepted.type === "subprocess") {
      setData((prev: TraceData) => {
        const subprocesses = appendBoundedUnique(prev.subprocesses, accepted.record, recordKeysRef.current.subprocesses);
        return subprocesses === prev.subprocesses ? prev : { ...prev, subprocesses };
      });
      return "accepted";
    }
    return "rejected";
