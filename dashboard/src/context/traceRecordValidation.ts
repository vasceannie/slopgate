...
function hasTraceIdentity(obj: Record<string, unknown>): obj is Record<string, unknown> & TraceIdentity {
  return typeof obj.timestamp === "string" && typeof obj.event_name === "string" && typeof obj.session_id === "string";
}
...
function optionalAliasedStringArray(obj: Record<string, unknown>, keys: string[]): string[] | undefined {
...
function normalizePlatformSource(value: unknown, rawPlatform: unknown): PlatformSource {
  switch (value) {
    case "explicit":
    case "defaulted":
    case "normalized":
    ...
    default:
      ...
      return normalizePlatform(rawPlatform) === "unknown" ? "normalized" : "explicit";
  }
}
...
function isEventName(value: unknown): value is EventName {
  return typeof value === "string" && EVENT_NAMES.some((eventName) => eventName === value);
}
...
function traceMetadata(obj: Record<string, unknown>, includeBareTitle = false) {
  const originPlatform = optionalAliasedPlatform(obj, ["origin_platform", "originPlatform"]);
  const lineageRole = normalizeLineageRole(aliasedValue(obj, ["lineage_role", "lineageRole"]));
  return {
    ...
    session_title_source: optionalAliasedString(obj, ["session_title_source", "sessionTitleSource"]),
    session_identity_source: optionalAliasedString(obj, ["session_identity_source", "sessionIdentitySource"]),
    opencode_session_id: optionalAliasedString(obj, ["opencode_session_id", "opencodeSessionId", "opencodeSessionID"]),
    ...
    secondary_session_ids: optionalAliasedStringArray(obj, ["secondary_session_ids", "secondarySessionIds"]),
    parent_session_id: optionalAliasedString(obj, ["parent_session_id", "parentSessionId", "parentSessionID"]),
    root_session_id: optionalAliasedString(obj, ["root_session_id", "rootSessionId", "rootSessionID"]),
    origin_platform: originPlatform,
    origin_session_id: optionalAliasedString(obj, ["origin_session_id", "originSessionId", "originSessionID"]),
    platform_source: normalizePlatformSource(aliasedValue(obj, ["platform_source", "platformSource"]), obj.platform),
    subagent_type: optionalAliasedString(obj, ["subagent_type", "subagentType"]),
    spawn_description: optionalAliasedString(obj, ["spawn_description", "spawnDescription"]),
    lineage_role: lineageRole,
  };
}

function aliasedValue(obj: Record<string, unknown>, keys: string[]): unknown | undefined {
...
function optionalAliasedString(obj: Record<string, unknown>, keys: string[]): string | null | undefined {
  const value = aliasedValue(obj, keys);
  return optionalString(value);
}

function optionalAliasedPlatform(obj: Record<string, unknown>, keys: string[]): Platform | null | undefined {
  const value = aliasedValue(obj, keys);
  if (value === undefined) return undefined;
  if (value === null) return null;
...
function normalizeRuleFindingRecord(obj: Record<string, unknown>): RuleFinding | null {
...
function normalizeFindingSummary(value: unknown): HookResult["findings"][number] | null {
...
export function coerceTraceRecord(obj: Record<string, unknown>): AcceptedTraceRecord | null {
  ...
  if (type === "rule") {
    const record = isRuleFindingRecord(obj) ? obj : normalizeRuleFindingRecord(obj);
    if (record) return { type, record };
  }
  if (type === "result") {
