...
export function FlagProvider({ children }: { children: ReactNode }) {
  ...
  const addFlag = useCallback((item: Omit<InvestigationFlag, "id" | "createdAt" | "resolved">) => {
    setFlags(prev => {
      const next = [...prev, {
        ...item,
  ...
  const unresolveFlag = useCallback((id: string) => {
    setFlags(prev => {
      const next = prev.map(f => f.id === id ? { ...f, resolved: false } : f);
      saveFlags(next);
      return next;
    });
  }, []);

  const isFlagged = useCallback((itemType: FlagItemType, itemId: string) => {
    return flags.some(f => f.itemType === itemType && f.itemId === itemId && !f.resolved);
  }, [flags]);

  const getFlagsForItem = useCallback((itemType: FlagItemType, itemId: string) => {
    return flags.filter(f => f.itemType === itemType && f.itemId === itemId);
  }, [flags]);
  ...
  const value = useMemo(() => ({
    flags, addFlag, removeFlag, resolveFlag, unresolveFlag, isFlagged, getFlagsForItem, exportFlags,
  }), [flags, addFlag, removeFlag, resolveFlag, unresolveFlag, isFlagged, getFlagsForItem, exportFlags]);
