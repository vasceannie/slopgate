...
describe("live trace record validation", () => {
  ...
  it.each(EVENT_NAMES)("accepts canonical platform event %s", (eventName) => {
    const rawEvent = {
      timestamp: "2026-06-11T02:02:33.860998+00:00",
...
function DataIdentityProbe() {
  ...
  return createElement("div", { "data-testid": "data-identity" }, `${data.rules.length}:${dataChanges}`);
}
...
function SnapshotSummaryProbe() {
  ...
  return createElement("div", { "data-testid": "snapshot-summary" }, JSON.stringify(sourceMeta.snapshotSummary ?? null));
}
...
function SourceActionProbe() {
  ...
  return createElement(
    ...
    createElement("button", { "data-testid": "reset", onClick: resetToMock, type: "button" }, "reset"),
    createElement(
      "button",
      {
        "data-testid": "upload",
...
describe("TraceDataProvider stream de-duplication", () => {
  ...
  it("ignores exact duplicate stream records", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("rule-count")).toHaveTextContent("1"));
  });

  it("keeps trace data identity stable for exact duplicate stream records", async () => {
    render(createElement(TraceDataProvider, null, createElement(DataIdentityProbe)));
    ...
    await waitFor(() => expect(screen.getByTestId("data-identity")).toHaveTextContent(/^1:/));
    ...
    expect(screen.getByTestId("data-identity").textContent).toBe(afterFirstRecord);
  });
  ...
  it("ignores non-trace stream JSON without counting schema failures", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("source-state")).toHaveTextContent("streaming:false:1:0:0"));
  });
});
...
describe("TraceDataProvider initial snapshot loading", () => {
  ...
  it("requests a bounded 24h snapshot on initial load", async () => {
    ...
    expect(fetchSnapshot).toHaveBeenCalledWith(expect.stringContaining("lookback_hours=24"));
  });
  ...
  it("starts empty while the first live snapshot is pending", async () => {
    ...
    expect(screen.getByTestId("source-state")).toHaveTextContent("mock:true:0:0:0");
  ...
  it("stores server-side snapshot summaries in source metadata", async () => {
    ...
    render(createElement(TraceDataProvider, null, createElement(SnapshotSummaryProbe)));

    await waitFor(() => expect(screen.getByTestId("snapshot-summary")).toHaveTextContent('"session_count":3'));
    expect(screen.getByTestId("snapshot-summary")).toHaveTextContent('"subprocess_failures":2');
  });
  ...
  it("replaces baked synthetic data with the first real snapshot", async () => {
    ...
    expect(screen.getByTestId("source-state")).toHaveTextContent("baked:true:1:0:0");
    await waitFor(() => expect(screen.getByTestId("source-state")).toHaveTextContent("streaming:false:0:0:0"));
  });
  ...
  it("ignores a stale snapshot after uploaded data is accepted", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("source-state")).toHaveTextContent("uploaded:false:1:0:0"));
    if (!resolveSnapshot) throw new Error("snapshot request was not started");
    ...
    expect(screen.getByTestId("source-state")).toHaveTextContent("uploaded:false:1:0:0");
  });
});
