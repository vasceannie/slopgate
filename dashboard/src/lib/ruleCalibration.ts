...
function recurrenceScore({
  ...
  if (rate <= 0) return 0;
  const intensity = asymptoticRatio(averageFindingsPerSession, FINDINGS_PER_SESSION_SCORE_SCALE);
  const breadth = asymptoticRatio(sessions, SESSION_BREADTH_SCORE_SCALE);
  return Math.min(
    100,
    Math.round(rate * (RECURRENCE_BASE_WEIGHT + intensity * RECURRENCE_INTENSITY_WEIGHT + breadth * RECURRENCE_BREADTH_WEIGHT)),
  );
}

export function computeCalibrationSignals(rules: RuleFinding[], results: HookResult[]): RuleCalibrationSignal[] {
  ...
  for (const rule_id of allRuleIds) {
    ...
    const warnCount = decisions.filter((d) => d === "warn" || d === "context" || d === "ask" || d === "info").length;
    ...
    const allowAfterWarn = sessionsWithRule.filter((sid) => {
      ...
      const hadWarning = ruleDecisions.some((d) => d === "warn" || d === "ask" || d === "context" || d === "info");
    ...
    const advisoryPressure = Math.min(100, Math.round((allowAfterWarnRatio * 60 + warnRatio * 40) * sampleSizeConfidence));
    ...
    for (const [sid, sessionFindings] of findingsBySession.entries()) {
      ...
      const deliveredResults = (resultsBySession.get(sid) || []).filter((result) => findingsForRule(result, rule_id).length > 0);
    ...
    const repeatFireRate = activeSessionsCount > 0 ? multiWarnSessionsCount / activeSessionsCount : 0;
    const repeatFirePressure = recurrenceScore({
      rate: repeatFireRate,
      averageFindingsPerSession: activeSessionsCount > 0 ? totalFindings / activeSessionsCount : 0,
      sessions: activeSessionsCount,
    });
    const runtimeErrorCount = errorsByRule.get(rule_id) || 0;
    const runtimeErrorRatio = asymptoticRatio(runtimeErrorCount, RUNTIME_ERROR_SCORE_SCALE);
    const runtimeErrorPressureFromErrors =
      runtimeErrorCount > 0 ? Math.min(100, RUNTIME_ERROR_BASE_SCORE + Math.round(runtimeErrorRatio * RUNTIME_ERROR_WEIGHT)) : 0;
    const runtimeErrorPressure = Math.max(repeatFirePressure, runtimeErrorPressureFromErrors);
    // 3. decisionVariance (repurposed as delivered persistence triage score)
    const persistenceRate = totalDeliveredCreated > 0 ? Math.min(1, totalPersistentDelivered / totalDeliveredCreated) : 0;
    const decisionVariance = recurrenceScore({
      rate: persistenceRate,
      averageFindingsPerSession: deliveredSessions > 0 ? totalPersistentDelivered / deliveredSessions : 0,
