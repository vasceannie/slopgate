...
function LazyTab({ children }: { children: ReactNode }) {
  return <Suspense fallback={<DashboardTabLoadingPlaceholder />}>{children}</Suspense>;
}
      ...
      <div className="px-6 pt-4 pb-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="fp-analysis" className="mt-4">
            <LazyTab>
              <LazyFalsePositiveAnalysis rules={data.rules} results={data.results} />
            </LazyTab>
          </TabsContent>
          ...
          <TabsContent value="ops" className="mt-4">
            <LazyTab>
              <div className="space-y-6">
                <OpsPostureStrip asyncFailCount={data.async.failCount} config={data.drift.config} harnessStatus={harnessStatus} />
                <div className={hasAsyncJobs ? "grid grid-cols-1 items-start gap-6 lg:grid-cols-2" : "grid grid-cols-1 gap-6"}>
