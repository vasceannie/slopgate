...
- **OpenCode**: plugin shim at the user config plugins dir and/or `.opencode/plugins/slopgate-plugin.ts`. Native events (`tool.execute.before`, `tool.execute.after`, `file.edited`, `permission.asked`, `permission.replied`, `session.created`, `session.compacted`, `session.idle`, `session.error`, `session.status`, `shell.env`, and `command.executed`) are forwarded into Slopgate's canonical model. Hard mutation control is owned by the documented `tool.execute.before` hook. Generic events are fire-and-forget telemetry: `file.edited` supplies delayed provisional evidence and can establish a repair requirement for later calls, but it cannot reject or roll back the edit that emitted it. Permission event spellings are version-sensitive and advisory; Slopgate does not depend on `permission.ask`. OpenCode exposes a typed `chat.message` hook, but no documented prompt-blocking contract, so prompt and `session.idle` guidance remain advisory. Compatibility plugins may also replay Claude-configured hooks; payloads marked `hook_source: "opencode-plugin"` retain OpenCode adapter semantics even when that compatibility command supplies `--platform claude`.
  Slopgate classifies OpenCode tools by exact normalized ID. Declared read-only tools remain available, declared effectful tools are forwarded to the normal policy engine, and every undeclared tool fails closed before execution regardless of its argument shape. During `REPAIR_REQUIRED`, only declared read-only tools, `write`/`edit`/`apply_patch`, the dedicated repair verifier, and an exact `slopgate lint check` command are allowed; opaque wrappers and compound shell commands remain blocked.
