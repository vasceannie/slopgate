...
class TestImportAliasGuard(unittest.TestCase):
    ...
    def test_nonstandard_module_import_alias_denied(self) -> None:
        ...
        assert_denied_by(result, "PY-IMPORT-002")
...
class TestImportFanout(unittest.TestCase):
    ...
    def test_at_threshold_ok(self) -> None:
        ...
        assert "PY-IMPORT-001" not in rule_ids, (
            "at-threshold import must not be flagged"
        )
    ...
    def test_over_threshold_context_only(self) -> None:
        ...
        assert_not_denied(result)
        rule_ids = {f.rule_id for f in result.findings}
        assert "PY-IMPORT-001" in rule_ids, (
            "over-threshold import must fire PY-IMPORT-001"
        )
    ...
    def test_bare_import_not_flagged(self) -> None:
        ...
        rule_ids = {f.rule_id for f in result.findings}
        assert "PY-IMPORT-001" not in rule_ids, (
            "bare import must not trigger PY-IMPORT-001"
        )
...
class TestPrivateImportPathGuard(unittest.TestCase):
    def test_existing_private_implementation_file_remains_editable(self) -> None:
        path = "src/slopgate/rules/python_ast/_rules/_boundary_rule.py"
        payload = {
            "hook_event_name": "PreToolUse",
            "tool_name": "Write",
            "tool_input": {"file_path": path, "content": "value = 1\n"},
            "cwd": str(BUNDLE_ROOT),
        }
        ...
        assert "PY-IMPORT-003" not in rule_ids, (
            "editing an existing private implementation must not introduce its path"
        )

    def test_patch_update_of_private_implementation_remains_editable(self) -> None:
        path = "src/slopgate/rules/python_ast/_rules/_boundary_rule.py"
        patch = (
            f"*** Begin Patch\n*** Update File: {path}\n@@\n+value = 1\n*** End Patch"
        )
        ...
        rule_ids = {finding.rule_id for finding in result.findings}
        assert "PY-IMPORT-003" not in rule_ids, (
            "patch updates must not reclassify an existing private path as new"
        )
