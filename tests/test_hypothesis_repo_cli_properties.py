...
def test_scope_paths_returns_expected_targets_property(
    ...
    with TemporaryDirectory() as raw_path:
        ...
        assert scope_paths(scope, user_path=user_path, project_path=project_path) == expected
...
def test_cmd_list_returns_zero_without_local_indexes(
    ...
    monkeypatch.setattr(search_cli, "load_config", lambda: search_config.SearchConfig())
...
def test_cmd_migrate_clean_repo_is_noop_property(
    tmp_path: Path,
    dry_run: bool,
) -> None:
    assert (
...
@given(
    scope=strategies.sampled_from(["user", "project"]),
    platform_label=SHORT_TEXT,
)
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
@given(
    path_text=strategies.one_of(
        strategies.none(),
        strategies.text(alphabet=" \t", max_size=3),
    ),
)
