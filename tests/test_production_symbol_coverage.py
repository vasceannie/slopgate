...
def test_python_ast_helpers_expose_public_entrypoints() -> None:
    ...
    assert {
        "decision": decision_for_context(DummyContext()),  # type: ignore[arg-type]
...
def test_search_config_helpers_expand_save_and_detect_provider(
    ...
    monkeypatch.setattr(search_config, "APP_CONFIG", config_path)
    monkeypatch.setattr(search_config, "APP_DIR", config_path.parent)
    ...
    assert json.loads(config_path.read_text(encoding="utf-8"))["islands_config"] == str(target)


def test_installer_shared_helpers_handle_missing_and_backup_paths(tmp_path: Path) -> None:
...
def test_cursor_adapter_exposes_public_render_and_normalize() -> None:
    ...
    normalized = adapter.normalize_payload({"hook_event_name": "preToolUse", "cwd": "/tmp"})
...
def test_search_cli_init_returns_zero_for_dry_init(
    ...
    monkeypatch.setattr(search_config, "APP_CONFIG", config_path)
    monkeypatch.setattr(search_config, "APP_DIR", app_dir)
