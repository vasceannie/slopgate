...
import pytest
from slopgate.search.config import IsxError, detect_provider, expand, save_config
...
def test_save_config_writes_json_file(
    ...
    monkeypatch.setattr(slopgate.search.config, "APP_CONFIG", app_config)
    data: slopgate.search.config.SearchConfig = {
        "provider": "ollama",
