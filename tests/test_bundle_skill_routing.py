...
def test_slopgate_skills_have_findable_activation_contract(skill_name: str) -> None:
    ...
    assert 80 <= len(description) <= 1024, (
        "description should be useful but loader-safe"
    )
    ...
    assert "slopgate:" in frontmatter, (
        "skill should expose Slopgate activation metadata"
    )
...
def test_routing_fragment_prefers_one_primary_skill() -> None:
    ...
    assert "one primary skill first" in fragment, (
        "agents need anti-buffet routing guidance"
    )
...
def test_bundle_skills_do_not_embed_source_checkout_paths() -> None:
    ...
    assert ".openclaw/workspace-hooker" not in combined, (
        "skills must not require source checkouts"
    )
    assert "/home/trav/.openclaw" not in combined, (
        "skills must be usable from installed bundles"
    )
