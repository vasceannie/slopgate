...
def test_enroll_repo_returns_tuple_for_missing_git_root_property(name: str) -> None:
    with TemporaryDirectory() as raw_path:
        root, written_roots = enroll_repo(
            Path(raw_path) / name, include_worktrees=False
        )
...
def test_choose_litellm_model_honors_explicit_model_property(
    base: str, explicit: str | None
) -> None:
    model, _, _ = runtime.choose_litellm_model(base, None, explicit_model=explicit)
    assert model == explicit if explicit is not None else model


@given(
    extra=strategies.dictionaries(
        strategies.text(min_size=1, max_size=8), SHORT_TEXT, max_size=2
    )
)
def test_runtime_env_merges_extra_env_property(extra: dict[str, str]) -> None:
    cfg = config.SearchConfig(base_url="https://llm.example", api_key_env="")
...
def test_logging_detectors_accept_minimal_module_property(name: str) -> None:
    source = (
        "from slopgate.util.logger import make_logger\nlogger = make_logger(__name__)\n"
    )
...
def test_basic_test_smell_detectors_accept_valid_test_property(name: str) -> None:
    parsed = [
        _parsed(f"def test_{name}():\n    assert 1 == 1\n", rel="tests/test_sample.py")
    ]
    assert (
        detect_assertion_free_tests(parsed),
...
def test_shell_command_paths_extracts_tokens_property(command: str) -> None:
    paths = shell_command_paths(command)
...
@given(command=SHORT_TEXT)
def test_command_is_slopgate_hook_detects_handle_invocation_property(
    command: str,
) -> None:
    result = _shared.command_is_slopgate_hook(command)
...
def _run_touched_collectors_sample(name: str) -> list[tuple[str, list[Violation]]]:
    with TemporaryDirectory() as raw_path:
        src_path = _write_sample_module(Path(raw_path), name)
        return _collectors.run_touched_collectors(
            [src_path], [src_path], reference_test_files=[src_path]
        )


def _run_all_collectors_sample(name: str) -> list[tuple[str, list[Violation]]]:
    with TemporaryDirectory() as raw_path:
        src_path = _write_sample_module(Path(raw_path), name)
        return _collectors.run_all_collectors([src_path], [src_path])


def _run_test_integrity_collectors_sample(
    name: str,
