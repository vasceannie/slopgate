...
def _evaluate_common_when_rule_disabled() -> list[object]:
    ...
    def _always_disabled(_ctx: object, _rule_id: str) -> bool:
        return False

    base_mod.is_rule_enabled = _always_disabled
    try:
        return evaluate_common(_StubRule(), ctx_patch, lambda src, path, c: [])
    finally:
