...
REFACTORED_PUBLIC_SYMBOLS_a: tuple[object, ...] = (
    rule_hint,
    failure_class,
    finding_path,
    denial_context,
    ...
    metadata_lines,
    prognosis,
    slopgate.lint._details._test_context.test_context_lines,
    dotted_name,
    call_tail,
    iter_tests,
    is_none_constant,
...
def test_refactored_public_symbols_are_callable_a(_: None) -> None:
    assert all(
        callable(symbol) or isinstance(symbol, type)
        for symbol in REFACTORED_PUBLIC_SYMBOLS_a
    )
