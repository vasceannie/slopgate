...
def _source_interop_collectors(parsed_src: list[ParsedFile]) -> CollectorResults:
...
def _code_smell_collectors(parsed_src: list[ParsedFile]) -> CollectorResults:
...
def _module_size_collectors(oversized: list[Violation]) -> CollectorResults:
    return [
...
def structure_src_collectors(
    ...
    return [
        *_source_interop_collectors(parsed_src),
        *_code_smell_collectors(parsed_src),
        *_module_size_collectors(oversized),
        *_duplicate_src_collectors(parsed_src, literals),
    ]
...
def source_analysis(src_files: list[Path], test_files: list[Path]) -> SourceAnalysis:
    ...
    from slopgate.lint.project_index import ProjectIndexRequest, build_project_index
