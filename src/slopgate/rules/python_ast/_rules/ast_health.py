"""Detect unparseable Python sources and oversized modules."""
...
class PythonAstHealthRule(Rule):
    ...
    def _recovery_text(path_value: str, kind: str) -> str:
        ...
        if kind == "read_error":
            return (
                "Next step: verify the path first with "
                f"`test -e {quoted_path}`; if it was moved/renamed, re-read the "
                "moved/renamed file before running syntax checks."
            )
        return (
