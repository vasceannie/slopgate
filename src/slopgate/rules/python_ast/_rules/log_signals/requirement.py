"""Require structured logging at event and package boundaries."""
...
from .classify import has_boundary_log_call, is_test_module_path
from .kinds import (
    BoundaryFunction,
...
class PythonBoundaryLoggingRule(Rule):
    ...
    def _boundary_context(ctx: HookContext) -> str:
        snippets = [
            ...
            "abstraction, not stdlib logging. For TUI/Textual lifecycle "
            "methods and package/event boundaries, import the logger factory "
            "or telemetry client your project already provides; define a "
            "module logger such as `logger = get_project_logger(__name__)` "
            "when that matches your project abstraction; then call "
            "`logger.info(...)` with operation/state/count/status fields "
            "before the handoff. Include event name, target package/service, "
            "correlation/request id when available, and enough stable fields "
            "to debug failures without logging secrets or raw payloads.",
        ]
        import_hint = ctx.config.hook_project_logger_import
        if import_hint:
            snippets.append(f"Project logger import: `{import_hint}`.")
        usage_hint = ctx.config.hook_project_logger_usage
        if usage_hint:
            snippets.append(f"Project logger usage: `{usage_hint}`.")
    ...
    def finding(
        ...
        return RuleFinding(
            ...
            message=(
                f"Python {boundary.kind} `{owner}` in `{path_value}` crosses "
                "an observable boundary without a logging/telemetry call. Add "
    ...
    def grouped_finding(
        ...
        return RuleFinding(
            ...
            message=(
                ...
                "logging. "
                "Add a module-level logger and log before the exported "
                f"boundary methods: {', '.join(owners)}."
            ),
            additional_context=self._boundary_context(ctx),
            metadata={
                METADATA_PATH: path_value,
    ...
    def _check_source(
        ...
        for node, class_name in iter_public_boundary_functions(module.body):
            ...
            if has_boundary_log_call(node):
                continue
            boundaries.append(
                BoundaryFunction(node=node, kind=kind, class_name=class_name)
            )
        if not boundaries:
            return []
