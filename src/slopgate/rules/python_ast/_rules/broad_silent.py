"""Detect broad except handlers and silent exception swallows."""
                ...
                if has_logger and not has_raise:
                    return [
                        RuleFinding(
                            ...
                            message=(
                                f"Broad exception handler in `{path_value}` logs "
                                "without re-raising. Catch specific exceptions or "
                    ...
                    if isinstance(stmt, (ast.Pass, ast.Continue)):
                        return [
                            RuleFinding(
                                ...
                                decision=decision_for_context(ctx),
                                message=(
                                    f"Silent broad exception swallow in `{path_value}`."
                                ),
                                metadata={METADATA_PATH: path_value},
                            )
                        ]
                    if isinstance(stmt, ast.Return) and is_empty_default_return(stmt):
                        return [
                            RuleFinding(
                                ...
                                decision=decision_for_context(ctx),
                                message=(
                                    f"Broad exception handler in `{path_value}` "
                                    "returns an empty default."
                                ),
                                metadata={METADATA_PATH: path_value},
                            )
                        ]
