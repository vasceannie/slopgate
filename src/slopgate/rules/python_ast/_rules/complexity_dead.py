"""Detect high cyclomatic complexity and unused Python definitions."""
    ...
    def _check_source(
        ...
        return [
            RuleFinding(
                ...
                message=(
                    f"Function `{worst_name}` in `{path_value}` has cyclomatic "
                    f"complexity {worst_cc}. Keep complexity at or below {limit}."
                ),
                metadata={
        ...
        for node in parsed_functions(source, ctx):
            ...
            findings.append(
                RuleFinding(
                    ...
                    decision=decision_for_context(ctx),
                    message=(
                        f"Function `{node.name}` in `{path_value}` has unreachable "
                        f"code after `{cause}` at line {lineno}."
                    ),
                    metadata={
                        METADATA_PATH: path_value,
