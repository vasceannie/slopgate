...
from slopgate.constants import DENY, PERMISSION_REQUEST, PRE_TOOL_USE, METADATA_COMMAND
from slopgate.models import RuleFinding, Severity
from slopgate.rules.base import Rule, is_rule_enabled
from slopgate.util.payloads import lower_path, shell_command_executable_paths
...
def sensitive_pattern_expr(pattern: str) -> str:
    """Return a regex expression for a configured sensitive path pattern."""
    if pattern in {".key", ".pem"}:
        return re.escape(pattern) + "(?=$|[^a-z0-9_])"
    has_meta = any((ch in META_CHARS for ch in pattern))
    return pattern if has_meta else re.escape(pattern)


def compile_sensitive_patterns(raw: list[str]) -> list[re.Pattern[str]]:
    ...
    """
    cache_key = tuple(raw)
    cached = compiled_sensitive_cache.get(cache_key)
    ...
    for raw_pattern in raw:
        ...
        if not stripped:
            continue
        expr = sensitive_pattern_expr(stripped)
        compiled.append(re.compile(expr, re.IGNORECASE))
    compiled_sensitive_cache[cache_key] = compiled
    return compiled
...
class SensitiveDataRule(Rule):
    ...
    def _match_in_paths(
        self, paths: list[str], compiled: list[re.Pattern[str]]
    ) -> str | None:
    ...
    def _match_in_command(
        self, command: str, compiled: list[re.Pattern[str]]
    ) -> str | None:
        ...
        for pattern in compiled:
            for m in pattern.finditer(lowered):
                rest = lowered[m.end() :]
                    ...
                    (i for i, ch in enumerate(rest) if ch in _WORD_BREAKS), len(rest)
                )
                tail = rest[:end]
                is_safe = any(
                    (
    ...
    def evaluate(self, ctx: "HookContext") -> list[RuleFinding]:
        if not is_rule_enabled(ctx, self.rule_id):
            return []
        compiled = compile_sensitive_patterns(ctx.config.sensitive_path_patterns)
        if not compiled:
            return []
...
def match_system_command(command: str, prefixes: list[str]) -> str | None:
    """Return '[command]' if command references a system path as a target."""
    lowered = command.lower()
    executable_paths = {
        lower_path(value) for value in shell_command_executable_paths(command)
    }
    separator = "(?:^|[\\s;|&(<>=])"
    terminator = "[^\\s;|&()<>'\\\"]*"
    for prefix in prefixes:
        normalized_prefix = prefix.replace("\\", "/")
        if not normalized_prefix.startswith("/") and (
            not re.match("^[a-z]:/", normalized_prefix)
        ):
        ...
        for match in re.finditer(pat, lowered):
            ...
            if is_exact_dev_null(matched_path):
                continue
...
class SystemProtectionRule(Rule):
    ...
    def evaluate(self, ctx: "HookContext") -> list[RuleFinding]:
        if not is_rule_enabled(ctx, self.rule_id):
            return []
        prefixes = [
            i.replace("\\", "/").lower() for i in ctx.config.system_path_prefixes
        ]
        if not prefixes:
            return []
        matched = match_system_path(ctx.candidate_paths, prefixes, ctx.cwd)
        if not matched and ctx.shell_command:
            matched = match_system_command(ctx.shell_command, prefixes)
        if not matched:
            return []
        return [
            RuleFinding(
...
def is_git_commit_command(tokens: list[str]) -> bool:
    return len(tokens) >= 2 and tokens[:2] == ["git", "commit"]


def is_git_no_verify_shortcut(token: str) -> bool:
    return token == "-n" or (
        token.startswith("-") and (not token.startswith("--")) and ("n" in token[1:])
    )


def detect_git_bypass(command: str) -> str | None:
    ...
    if is_git_commit_command(tokens) and any(
        (is_git_no_verify_shortcut(token) for token in tokens[2:])
    ):
        return GIT_NO_VERIFY_SHORTCUT
...
class GitNoVerifyRule(Rule):
    ...
    def evaluate(self, ctx: "HookContext") -> list[RuleFinding]:
        ...
        if not ctx.shell_command:
            return []
        bypass = detect_git_bypass(ctx.shell_command)
        if not bypass:
            return []
        msg = f"Git hook bypass detected: `{bypass}`. Pre-commit and pre-push hooks exist for a reason — they run linters, type checks, and tests.\n\nIf hooks are failing, fix the issues they found rather than skipping them. Next step: run the hook/quality command normally, keep the full error visible, fix the hook/test failure, then commit without bypass: `git commit -m <message>`."
        return [
            RuleFinding(
                rule_id=self.rule_id,
                title=self.title,
