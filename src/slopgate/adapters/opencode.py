...
class OpenCodeAdapter(PlatformAdapter):
    ...
    def normalize_payload(self, raw: ObjectMapping) -> ObjectDict:
        logger.debug("OpenCode adapter metric", metric="opencode.normalize_payload")
        ...
        merge_standard_session_fields(
            raw,
            canonical,
            cwd_extra_keys=("worktree", "directory"),
        )
        if oc_event == "tool.execute.before":
            tool_input = object_dict(canonical.get("tool_input"))
            normalized_tool_input = normalize_projected_tool_input(
                ProjectionRequest(
                    tool_name=tool_name,
                    tool_input=tool_input,
                    root=Path(string_value(canonical.get("cwd")) or "."),
                    contract_version=(
                        string_value(raw.get("opencode_tool_contract_version")) or ""
                    ),
                )
            )
    ...
    def _block_output(
        ...
        logger.debug("OpenCode adapter metric", metric="opencode.render.block")
    ...
    def _context_output(context: str) -> ObjectDict:
        logger.debug("OpenCode adapter metric", metric="opencode.render.context")
        return {"action": "context", "context": context}

    def _render_pre_tool_use(
        self, request: _OpenCodeRenderRequest
    ) -> ObjectDict | None:
        logger.debug("OpenCode adapter metric", metric="opencode.render.pre_tool_use")
    ...
    def _render_permission_request(
        ...
        logger.debug(
            "OpenCode adapter metric", metric="opencode.render.permission_request"
        )
        if request.decision in {DENY, BLOCK, ASK}:
    ...
    def _render_post_tool_use(
        ...
        logger.debug("OpenCode adapter metric", metric="opencode.render.post_tool_use")
        payload: ObjectDict = {}
    ...
    def _render_session_start(
        ...
        logger.debug("OpenCode adapter metric", metric="opencode.render.session_start")
        if request.context:
            return self._context_output(request.context)
        return None

    def _render_stop(self, request: _OpenCodeRenderRequest) -> ObjectDict | None:
        logger.debug("OpenCode adapter metric", metric="opencode.render.stop")
    ...
    def render_output(
        ...
        **kwargs: object,
        ...
        logger.debug("OpenCode adapter metric", metric="opencode.render_output")
