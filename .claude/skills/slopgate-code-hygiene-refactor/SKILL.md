---
name: slopgate-code-hygiene-refactor
description: |
  Recovery playbook for Slopgate and project quality failures. Load after hook/lint denials such as PY-CODE-017 flat sibling modules, PY-CODE-018 oversized modules, QUALITY-LINT-001 post-edit quality backstops, PY-QUALITY-010 magic numbers / repeated literals, PY-CODE-012/PY-CODE-013 smell findings, type-suppression bans, duplicate helpers, or repeated hygiene retry loops. Also load when asked to "refactor this module", "split this file", "extract a class", "reduce complexity", "fix god class", "flatten module hierarchy", "move code to a package", "extract helpers", "consolidate duplicates", "replace magic numbers with constants", or when a hook blocks an edit with structural complaints. Focuses on preserving public APIs while refactoring toward packages, cohesive modules, typed seams, and focused tests.
version: 1.0.0
author: Slopgate
license: MIT
...
metadata:
  hermes:
    tags: [slopgate,refactor,module-size,complexity,quality-gates]
    related_skills: [slopgate-hygiene-orchestrator,slopgate-code-smell-utility-locator,slopgate-intelligent-coding-patterns]
  slopgate:
    rule_ids: [PY-CODE-017,PY-CODE-018,PY-CODE-014,PY-CODE-008,PY-CODE-009,PY-CODE-015,QUALITY-LINT-001]
    activation:
      primary: [oversized modules/functions,flat sibling packages,god classes,post-edit quality failures]
      avoid: [repo-wide coordination,helper inventory before ownership is known,test-only expansion]
---
...
Use for a local structural repair after a hook, lint, CI, or review says the code shape is wrong.

- Rule IDs include `PY-CODE-017`, `PY-CODE-018`, `PY-CODE-014`, complexity/parameter/function-size findings, or touched-file `QUALITY-LINT-001`.
- The likely fix is splitting modules, extracting cohesive helpers/classes, preserving public imports, or repairing a post-edit quality backstop.
- Use after `slopgate-code-smell-utility-locator` if the first question is whether an existing helper/constant already owns the behavior.

## When Not to Use

Do not use as the first skill for broad repo-wide cleanup; use `slopgate-hygiene-orchestrator` to group and dispatch.

Do not use for simple helper discovery, magic-number ownership, or duplicate utility inventory before code ownership is known; use `slopgate-code-smell-utility-locator`.

Do not use for test-only coverage gaps unless production structure must change first.
...
### `PY-QUALITY-010` — magic numbers / repeated literals

Signal: a literal value appears in code outside a constants/config module, or the same literal is repeated across call sites.

Recovery:

1. Check the hook/lint output for the cited existing constant first. Good output names the symbol and owner, e.g. `PRIMARY_TIMEOUT_MS = 500 (src/constants.py:12)`.
2. Read that owner file before editing. Confirm the constant is semantically the same value/policy, not just coincidentally equal.
3. Import and reuse the existing symbol at call sites; preserve the owner module as the single source of truth.
4. If no existing constant is cited, create one near the owning domain (`constants.py`, `_constants.py`, config/defaults module, or a domain package constant), then replace all semantic duplicates intentionally.
5. For magic numbers in algorithmic/math code, prefer named constants that explain the policy, not just the value: `MAX_RETRIES = 3` not `THREE = 3`.
6. Verify with the focused test/compile command and repo-root `slopgate lint check`.
