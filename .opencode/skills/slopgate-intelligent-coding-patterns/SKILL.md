---
name: slopgate-intelligent-coding-patterns
description: |
  Pattern-selection playbook for agents editing code with complex branching, stateful parsers/workflows, duplicated skeletons, policy checks, or ordered transforms. Load before refactoring if/elif dispatch chains, boolean-flag state machines, repeated validation predicates, parallel provider/language implementations, or regex/content transformation pipelines. Also load when asked to "apply a design pattern", "which pattern should I use here", "refactor this switch statement", "replace if-elif chain", "state machine pattern", "strategy pattern", "template method", "pipeline pattern", "specification pattern", "dispatch table", "guard clause refactor", "reduce cyclomatic complexity", or when Slopgate fires PY-CODE-014, PY-CODE-017, PY-CODE-018, QUALITY-LINT-001, or PY-QUALITY-010. Use with behavior-locking tests and avoid pattern theater for simple one-off code.
version: 1.0.0
author: Slopgate
license: MIT
...
metadata:
  hermes:
    tags: [slopgate,design-patterns,dispatch,state-machines,refactor]
    related_skills: [slopgate-code-hygiene-refactor,slopgate-code-smell-utility-locator,slopgate-test-extender]
  slopgate:
    rule_ids: [PY-CODE-012,PY-CODE-013,PY-CODE-014,PY-CODE-017,PY-CODE-018,PY-QUALITY-010,QUALITY-LINT-001,QUALITY-PATTERN-002]
    activation:
      primary: [pattern selection,dispatch chains,state machines,policy predicates,pipelines]
      avoid: [two-branch cleanup,style-only edits,speculative abstraction]
---
...
Use when the next edit requires choosing a design pattern rather than simply moving code.

- Code has three or more branches, dispatch by key/type/provider/event, boolean-flag parser/workflow state, repeated policy predicates, or ordered transforms.
- User asks which pattern to use, or mentions strategy/state/specification/template-method/pipeline/dispatch-table refactors.
- Pair with behavior-locking tests before the pattern change.

## When Not to Use

Do not use for simple two-branch cleanup, renames, formatting, or speculative future extensibility.

Do not use merely because a Slopgate quality rule fired; use `slopgate-code-hygiene-refactor` first unless the repair genuinely requires choosing among patterns.

Do not introduce a pattern that makes the happy path harder to read.
...
## Slopgate rule-to-pattern trigger map

Load this skill when Slopgate fires these rules:

| Slopgate Rule | Pattern to Apply | Section |
|---------------|------------------|---------|
| `PY-CODE-014` god class | Extract collaborator by data + responsibility | State Pattern / Template Method |
| `PY-CODE-017` flat siblings | Package split with `__init__.py` facade | Dispatch Table (for package API) |
| `PY-CODE-018` oversized module | Identify seams → package split | Pipeline / Template Method |
| `QUALITY-LINT-001` long-method | Extract helpers by phase | Pipeline / Guard Clauses |
| `QUALITY-LINT-001` repeated-code-block | Extract shared helper/service | Template Method + Strategy |
| `PY-QUALITY-010` magic numbers | Specification Pattern for policy constants | Specification Pattern |
| `PY-CODE-012` thin wrappers | Inline or add real boundary | Guard Clauses / Specification |
| `PY-CODE-013` feature envy | Move behavior to envied owner | State Pattern / direct method |
